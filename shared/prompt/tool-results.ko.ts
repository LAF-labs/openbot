...
export const TOOL_RESULT_KO: Record<string, string> = {
  ...
  /*
   * 서버의 컴퓨터 클라이언트와 라우트가 내는 사실들 (2파동, 2026-09-14). 예전에는 영어 문장이었다 —
   * 클라이언트 자신의 "The assistant's computer is not running."이나, 컨테이너가 쓴 워크스페이스
   * 문장("There is no file at notes.md."), Playwright의 호출 기록까지 — 그리고 그 문장이 그대로
   * 모델의 툴 결과가 됐다. `computer-routes-codes.test.ts`가 봇의 툴이 만날 수 있는 코드마다 여기
   * 문장이 있는지 서버 소스를 읽어 확인한다.
   */
  ...
  "laf:computer_unavailable":
    "봇의 컴퓨터가 지금 응답하지 않는다. 네가 고칠 수 있는 것이 아니니 같은 행동을 되풀이하지 말고, 컴퓨터가 응답하지 않는다고 사람에게 알려라.",
  "laf:snapshot_stale":
    "네가 가진 ref는 지금 화면의 것이 아니다. computer_snapshot을 새로 찍고, 새로 받은 ref와 snapshotId로 다시 해라.",
  // 시간 초과가 아닌데 페이지가 열리지 않은 것: 없는 주소, 연결을 받지 않는 사이트.
  "laf:page_failed":
  ...
  "laf:workspace_path_refused":
    "그 경로는 네 작업 공간 밖을 가리켜서 쓸 수 없다. 작업 공간 안의 상대 경로(예: notes/today.md)만 쓸 수 있고, /로 시작하는 절대 경로나 '..'은 쓸 수 없다. 같은 경로로 다시 시도하지 마라.",
  // 경로는 괜찮은데 거기서 쓸 것이 없던 것. 다른 요청이면 된다.
  "laf:workspace_file_unusable":
    "그 경로에서 쓸 수 있는 파일을 찾지 못했다 — 파일이 없거나, 파일이 아니라 폴더이거나, 한도보다 크다. computer_list_files로 무엇이 있는지 확인하고, 경로를 고치거나 내용을 줄여서 다시 해라.",
  ...
  "laf:navigation_failed":
    "그 주소를 열지 못했다 — 주소를 찾을 수 없거나 연결이 거절됐다. 같은 주소를 거듭 다시 열지 말고, 열리지 않았다고 사람에게 말해라.",
  "laf:browser_failed":
  ...
  "laf:file_path_refused":
    "그 경로는 네 작업 공간 밖이라 쓸 수 없다. 작업 공간 안의 상대 경로(예: notes/메모.md)만 쓸 수 있으니, 같은 경로를 다시 시도하지 마라.",
  "laf:file_not_found":
