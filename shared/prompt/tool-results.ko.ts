...
export const TOOL_RESULT_KO: Record<string, string> = {
  ...
  // 이 기계가 브라우저의 바깥 차단 규칙(호스트 방화벽)을 걸고 있지 않아 브라우저를 열지 않은 것.
  "laf:egress_unguarded":
    "봇의 브라우저를 지키는 보안 규칙이 이 기계에 걸려 있지 않아서 브라우저를 열지 않았다. 네가 고칠 수 있는 것이 아니니 다시 시도하지 말고, 브라우저를 지금 쓸 수 없다고 사장님께 그대로 알려라.",
  ...
  // 봇은 스스로 지우지 못한다. 잊고 고치는 것은 사람의 몫이고 수첩에 있다 — 그래서 부탁한다.
  /*
   * 사장님이 수첩에서 잊게 한 줄을 봇이 그대로 다시 적으려 했다. 사장님이 그 말을 한 메시지는 그날
   * 대화에 아직 있을 수 있어서, 읽은 봇이 되살리기 쉽다. 되살릴지는 사장님이 정한다.
   */
  "laf:memory_forgotten":
    "사장님이 수첩에서 잊게 한 내용이라 다시 적지 않았다. 대화에 그 말이 남아 있어도 기억으로 되살리지 않는다. 사장님이 다시 기억하길 원하면 수첩에 직접 적을 수 있다고 말해라.",
  "laf:memory_full":
    "기억이 가득 차서 적지 못했다. 네가 직접 지울 수는 없다 — 사장님께 수첩에서 오래된 기억을 잊거나 줄여 달라고 말하고, 그 전까지는 새로 적으려 하지 마라.",
  ...
  /*
   * 메일 툴 결과 끝에 한 번 붙는 줄(`server/src/plugins/mail-secrets.ts`). 인증번호·재설정 링크·로그인
   * 링크는 대화에 들어오면 매 턴 다시 읽히고, 같은 메일함의 다른 메일이 그걸 쓰라고 시킬 수 있다. 그래서
   * 값 대신 표시만 남고, 값은 사장님 화면에만 있다. 매 턴 따라다니는 문장이라 짧게.
   */
  "laf:mail_secret_withheld":
    "[[withheld:…]]는 인증번호·비밀번호 재설정·로그인 링크를 가린 자리다. 값은 사장님 화면의 '보기'에만 있으니 알아내려 하지 말고, 필요하면 사장님께 눌러 보시라고 해라.",
  "laf:mail_secret_kept_nowhere":
    "[[withheld:…]]는 인증번호·비밀번호 재설정·로그인 링크를 가린 자리다. 무인 실행이라 값은 어디에도 남기지 않았으니 알아내려 하지 마라.",
  // 사장님의 '보기'만 받는 답이라 봇에게 닿지 않는다. 그래도 plugins 아래 코드는 모두 여기 문장이 있다.
  "laf:withheld_gone":
    "가려 둔 인증번호나 링크는 시간이 지나 사라졌다. 다시 찾지 말고, 필요하면 새로 받아 달라고 사장님께 말해라.",
