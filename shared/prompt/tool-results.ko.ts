...
export const TOOL_RESULT_KO: Record<string, string> = {
  ...
  "laf:tool_arguments_invalid":
    "호출 인자가 그 툴의 정의와 맞지 않는다 — 필수 인자가 빠졌거나, 정해진 값 중 하나가 아닌 값이 있다. 사람에게 묻기 전에 거절했으니 아무 일도 일어나지 않았다. 툴 정의의 인자 설명을 다시 읽고 거기에 맞춰 고쳐서 다시 불러라. 모르는 값은 지어내지 말고 사람에게 물어라.",
  /*
   * 봇 서비스(agent-bot)가 모델의 답이 중간에 끊긴 실행을 끝낼 때, 인자가 다 오지 않은 호출에 붙인다.
   * 실행되지 않았고, 이 실행은 실패로 끝난다(감사 2026-09-10 A2, 8행).
   */
