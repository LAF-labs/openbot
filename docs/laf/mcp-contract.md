...
| `x-laf/effect: "external"` 또는 `"money"` | **작성된 정책이 허용해도 사람에게 묻는다.** 발신·결제·게시·삭제 — 규칙이나 봇의 자동 검토로는 넘어가지 않는다. 다만 사람이 그 카드에서 **이 도구 항상 허용**을 누르면 그 봇의 이후 호출은 인자와 상관없이 묻지 않는다(버튼이 말하는 그대로, 2026-09-16 결정). 카드에 호출 인자는 아직 보이지 않는다 — 받는 사람·내용을 보여 주는 미리보기는 LAF 내장 커넥터에만 있다 |
| `idempotentHint: true` | 실패 시 자동 재시도 허용 |
| (어노테이션 없음) | **destructive 취급** — 호출마다 사람에게 묻는다. **이 도구 항상 허용**을 받은 뒤로는 묻지 않는다 |
