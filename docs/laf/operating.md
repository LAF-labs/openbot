...
| `reply_cut`                    | agent-bot        | The provider's stream stopped before the model finished, with some of the answer already sent. `reason` is `ended_without_finish` (the body ended with no finish reason — a proxy's idle limit, a redeploy) or `failed_midway` (the read threw; `failure` says what kind). `chars` is how much arrived. The half stays in the transcript and the run fails with `laf:provider_stream_cut`. |
| `tool_call_answered`           | agent-bot        | A call the loop answered itself instead of forwarding. `code`: `laf:tool_unknown` (a name the run was never handed), `laf:tool_arguments_invalid` (not a JSON object), `laf:tool_loop` (the same call a third time in a row), `laf:tool_budget_spent` (the question's token budget). The model is asked again; when it will not recover, `run_failed` follows with the same `code` and `reason: guard`. |
