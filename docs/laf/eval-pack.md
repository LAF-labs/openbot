...
| 차원 | 시나리오 | 재는 것 |
|---|---|---|
| tool-calls | navigate / remember-vs-update_profile 쌍 / list-before-guessing / 12걸음 예산 / 다리로 메일 보내기 | 맞는 툴을, 유효한 인자로. remember 쌍은 실배포에서 실제로 터졌던 그 문장 그대로다. 12걸음은 앞선 툴 결과가 잘려서 프롬프트 토큰이 예산 안에 남는지를 잰다. 메일 보내기는 스키마에 없는 지메일 툴을 `tool_search`로 찾아 `tool_call`로 부르는지를 잰다(아래) |
