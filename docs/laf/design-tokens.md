...
## 3. 글자 — 스케일은 6칸, 한글에 맞춘 값 (2026-09-24)
...
| 클래스 | 크기/행간 | 쓰는 곳 |
|---|---|---|
| `text-xs` | 12 / 17 | 메타, 라벨, 배지, sm·xs 버튼 |
| `text-sm` | 13 / 19 | 목록 행, 메뉴 항목, 보조 문장, 카드 제목 |
| `text-base` | 14 / 21 | 본문 |
| `text-lg` | 17 / 25 | 섹션 제목 |
| `text-xl` | 22 / 30 | 다이얼로그·화면 제목 |
| `text-2xl` | 26 / 34 | 온보딩 헤드라인 |

채팅 본문(`--chat-font-size` / `--chat-line-height`)은 14 / 22다. 봇의 답은 한글 문단이고,
22px에서 두 줄이 서로 닿지 않는다.

**왜 바꿨나.** Grok에서 잰 값은 라틴 글자용이었다: 11px 캡션, 1.4 행간. 한글은 네모난 덩어리라
11px의 "10분 남음"은 뭉개지고, 1.4에서는 봇의 두 줄이 붙는다. 바닥은 12px, 행간은 크기의 약
1.5배다. **칸의 이름과 개수는 그대로다** — `text-xs`는 여전히 캡션이고, 이제 읽히는 캡션일 뿐이다.
`text-[12px]`은 이제 `text-xs`다. 새 칸은 만들지 않는다.

**글꼴은 한 줄이다.** `html`의 `font-sans` 하나를 body가 물려받는다: Pretendard(있는 사람에게만)
→ 번들된 Inter Variable(라틴·숫자) → Apple SD Gothic Neo·Malgun Gothic(한글). 전에는 body가
`Inter`를 불렀는데 번들은 "Inter Variable"로 등록되어, Inter가 깔린 기계가 아니면 앱 전체가 Apple SD
Gothic Neo로 그려졌다. 네트워크 글꼴은 없다.
...
| 클래스 | 값 | 토큰 | 쓰는 곳 |
|---|---|---|---|
| `h-row` | 54px | `--spacing-row` = `--sand-row-height` | 사이드바 행, 설정·관리자 레일 행 |

`--sand-row-height`는 나머지 기하 값과 함께 실측해서 적어 뒀는데, **부를 이름이 없었다.**
읽는 곳은 로스터 행 하나뿐이었고 그마저 `h-[var(--sand-row-height)]` — 드리프트 테스트가
세는 바로 그 탈출구다. 나머지는 전부 짐작했다: 설정 레일과 관리자 레일은 `h-10`(40px)이었고,
그 옆 주석은 "앱 사이드바와 같은 리듬"이라고 적혀 있었다. **한 화면은 54, 두 화면은 40.**

`--spacing-*` 네임스페이스에 넣었으므로 `h-row` 말고 `min-h-row`, `py-row`, `gap-row`도
나온다. `h-[var(--sand-row-height)]`은 이제 쓰지 않는다 — 이름이 있는 칸을 손으로 적는 것은
드리프트고, 이 칸은 아무도 적지 않아서 더 나빴다.
...
| `shadow-card` | `--sand-shadow-card` | 1px + 6px 얕은 2겹 | **없음** (카드 테두리가 헤어라인) |
...
`shadow-card`는 2026-09-24에 생겼다. 대화 안의 카드 넷(브라우저, 승인, 도움 요청, 루틴)은
`components/ui/card-surface.ts`의 `chatCard` 하나로 그린다 — 모서리 16, 폭 `max-w-md`,
`bg-card`, 헤어라인, 이 그림자. 사람을 기다리는 동안만 테두리가 호박색(`chatCardWaiting`)이 된다.
봇의 색이 아니라 호박색인 이유: "사장님 차례"는 신호라서, 빨간 봇의 보통 버튼처럼 보이거나 노란 봇
위에서 사라지면 안 된다.
...
**2026-09-24부터 링은 봇의 색이다** (`--ring` = `--bot-accent`). 그래서 대비를 지는 쪽이 바뀌었다.
전의 링은 40% 회색의 50% 후광이라 바탕과 약 1.3:1 — 표시가 아니라 암시였다. 지금 3:1을 지는 것은
**불투명한 부분**이다: 컨트롤은 색이 바뀐 테두리, 행은 안쪽 링 전체(`ring-ring`, 알파 없음).
컨트롤 둘레의 후광은 같은 색의 35%이고, 선을 찾기 쉽게 할 뿐 대비를 지지 않는다.
...
### 봇의 색이 앱의 강조색이다 (2026-09-24)

사람이 얼굴에 고른 색이 누를 수 있는 모든 것의 색이다: 기본 버튼, 보내기, 포커스 링, 선택 영역,
링크. `<html data-accent="orange">`가 팔레트를 이름으로 대고(`lib/avatar/accent.ts`가 봇의 시드에서
정한다), `styles.css` 끝의 팔레트별·테마별 블록이 네 값을 준다:
...
모든 바탕에서 4.6:1을 넘는 가장 밝은(다크는 가장 어두운) 점을 골랐다. 그래서 한 값이 버튼이자 링이자
링크다. `app/tests/accent-contrast.test.ts`가 `styles.css`를 다시 읽어 모든 쌍을 잰다. 팔레트를 더하면
블록도 더해야 테스트가 통과한다.

- 봇이 없을 때(로그인, 첫 실행 전)는 `data-accent`가 없고 예전의 중성 검정 컨트롤이다.
- 첫 실행에서 색을 고르는 동안은 그 색으로 미리 그린다(`useAccentPreview`).
- `index.html`이 마지막 값(`localStorage["laf-accent"]`)을 첫 페인트 전에 붙인다. 없으면 목록이
  답할 때마다 창 안의 버튼이 전부 검정에서 다시 칠해졌다.
- **봇이 부른다는 파란 점(`bg-mark`)은 봇의 색을 따르지 않는다.** 얼굴 위에 얼굴 색으로 그린 신호는
  보이지 않는다. "사장님 차례"의 호박색(`--warning`)도 같은 이유로 따로 있다.
