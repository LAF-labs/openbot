...
남은 것(진행 중 또는 대기): 봇 두뇌 v2(§5.2), 플러그인 분할·SSRF·OAuth 1회용(§5.4 b), 데스크톱 셸(§5.6 f), 정산 절차 통합·구조화 질문(§5.1 b·c), 상태 자리 결정 반영(§7-1: DB 승인·반복 변형과 LISTEN/NOTIFY 삭제), 브라우저 현실화(§5.3), 표면 분리·어휘(§5.6 a·b·d·g), 알림 아웃박스(§5.7), 대화 저장·데이터 모델(§5.4 e, §5.5), PIPA 엔드포인트(§5.9), 스모크 부활(§5.8). 미루기로 한 것: 첫 방문 호스트 ask(§5.1 a-5).
