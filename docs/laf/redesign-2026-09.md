...
| 2026-09-03 | cb9eb17 | §7-1 반영: 승인 대기·반복 카운트는 메모리(DB 변형 삭제, 마이그레이션 0025로 테이블 3개 DROP), 룸의 승인 대기는 폴링 대신 `waitFor` 프로미스(실측: 승인 조회 20회에 DB 문장 0개, 이전 40개), LISTEN/NOTIFY 제거(커밋 후 in-process 전달), "여러 서버" 주석 일소, deployment-model.md에 프로세스 내 상태 전체 목록 | 1416 |
