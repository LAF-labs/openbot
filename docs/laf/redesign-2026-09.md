...
남은 것(진행 중 또는 대기): 정산 절차 통합·구조화 질문(§5.1 b·c), 브라우저 현실화(§5.3), 표면 분리·어휘(§5.6 a·b·d·g), 알림 아웃박스(§5.7), PIPA 엔드포인트(§5.9), `server/tests` 타입 검사 편입, 플로어 상향. 미루기로 한 것: 첫 방문 호스트 ask(§5.1 a-5).
