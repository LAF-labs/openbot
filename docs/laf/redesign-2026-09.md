...
| 2026-09-05 | 553676e | **봇 아바타 생성기**(Grok: 단순한 도형 + 표정 있는 눈, 상태를 표정으로): 외부 그림 35개(`mascot-art.ts`, NOTICE) 삭제 → `f:<shape>.<palette>.<eyes>.<accessory>` 시드 문법(모양 6·팔레트 10·눈 4·장식 7 = 1,680), 다른 문자열은 소금 친 FNV-1a 4회로 매핑(기존 봇 얼굴 안정, 핀 테스트 4개). `BotAvatar`(JSX SVG, 20~144px, `idle/working/blocked/done` CSS 애니메이션·28px 미만과 reduced-motion은 정지), 고르기 창(미리보기 128 + 다른 얼굴 + 모양·색·장식 세 줄), `Mascot`/`MascotPicker` 래퍼 유지. 사이드바 행이 `working.ts`·`openQuestions()`로 상태를 넘김(회전 링 제거, 읽지 않음 점 유지, blocked는 `sr-only` status). 채널 아바타: 숨김·삭제 참여자도 시드로 얼굴(회색 원 없음). 스크린샷으로 잡은 것 6: 깊이 타원의 수평선, 아래를 보던 반달 눈, 머리 밖으로 나간 모자챙, 14px 루트에서 색 줄 줄바꿈, 배경색 뒤 투명 얼굴(`mascotBackground`→transparent), TDZ 백화면. 게이트 1976(app 373) | 실제 `working` 신호는 로컬 스택에서 관측 못 함(강제 prop으로 확인) |
| 2026-09-05 | b87a3ad | **봇 생성·프로필 재구성**(Grok: 누르면 즉시 생김, 봇 메뉴): "새 봇" → 한국어 이름(`BOT_NAME_WORDS`)·무작위 시드로 즉시 생성 → 그 봇의 대화로(`createBotNow`), 빈 대화 위 프로필 카드(얼굴·이름·하는 일·프리셋 칩; `pointer-events-none` 오버레이 때문에 칩이 죽어 있던 것을 눌러서 발견·수정). `/api/me`가 `deployment.seats` 보고, "내 봇 n/5"(숨김 포함), 5면 새로 만들기·복제 차단·4면 경고. 프로필 창 400px: 배너·⋯ 메뉴(편집·숨기기·복제·삭제, 결과를 항목에)·일하는 방식(저장됨)·속도·묻지 않아도 되는 일(한 문장+예시 칩 3, PATCH만)·기억(빈 상태)·스킬(봇별 스위치, 기존 `requireUser` 스코프 그대로, revoke 테스트 2 추가)·알림. 폼: 직함·역할 선택(서버와 일치 — 이름만 있는 봇을 못 고치던 회귀 수정), 공개/비공개 제거, 주소·키는 `/admin/bots`로, 어휘 예외 3개 삭제. `new-agent.tsx` 삭제, 웰컴 2화면(둘째는 버튼, 연결 링크 실제). 리베이스 충돌 4(import·프로필 카드) 수동 해결 + 합류 수정(카드 배경 inline style 제거, 웰컴 얼굴 3개 생성기, 고아 태그). 합친 게이트 2007(server 1395/플로어 1353·app 402/389·agent-computer 132·root 78) | 카드의 작업 중 배지는 로컬에서 `/api/copilotkit` 404라 미관측; 숨긴 봇을 목록에서 되살리는 길은 아직 없음(기존) |
| 2026-09-06 | 6b3941b | **아바타를 Grok Bot 그대로**(김기범: "Grok Bot 파일을 뜯어서 읽고 최대한 따라가, 네가 직접"): `/Applications/Grok Bot.app` 0.30.0의 `app.asar`를 풀어 렌더러 번들에서 아바타 엔진을 읽음 — 몸통 18종(기본 8종)은 전부 공식(초타원·원 합집합·둥근 다각형, 블롭만 손그림 패스), 정규화 228.44·중심 114.2705, 얼굴 자리는 내접 타원 탐색, 색 11종(밝은/어두운 값 쌍), 눈은 스타디움 슬릿 25표정(점 목록을 Grok 자신의 `fd` 피팅으로 5수치화), 눈이 몸통 표면을 따라 도는 3D 스핀, 상태별 스프링 목표(sleeping·waking·idle·listening·thinking·searching·working + 반응 16), 깜빡임 5키프레임, 윙크, 시선 난수 일정, 제스처(spinBounce·spinDizzy·spinWild·바운스 4단). 우리 구현: `grok-shapes.ts`(생성기 포트)·`grok-eyes.ts`(피팅 데이터+블렌드)·`grok-engine.ts`(rAF 엔진, 시간 주입 가능)·`BotAvatar`(눈은 페이지가 비치는 구멍 = `--background`, 몸통 색은 `.dark`로 갈리는 CSS 변수)·고르기 창은 모양 18·색 10 두 줄만, 시드 `s:<shape>.<colour>`(옛 `f:/g:`·자유 문자열은 Grok의 해시 그대로 매핑). 사이드바 무드: 질문 대기→notifying(배지), 작업→working, 끝난 직후 2초→happy, 30분 조용→sleeping(`bot-mood.ts`). 숨김 탭 복귀 시 밀린 시계를 다시 흩뿌림(전원이 동시에 깜빡이던 것을 스크린샷이 잡음). 헤드리스 캡처로 다크·라이트, working·notifying 강제 신호, 표정 5종 확인. 게이트 2005(app 400/플로어 389) | 장식·눈 선택 없음(김기범 규칙). Grok의 morph 레이어(dots·orbit·radar 등)와 색종이는 옮기지 않음 |
| 2026-09-06 | 5ab8554 | 몸통 12종(직각으로 잘리는 원기둥·결정·돔·아치·잎과 콩 삭제, 6개씩 두 줄), 얼굴 시험대 아티팩트(모양·색 선택, 미리보기 하나가 25상태 자동 순환, 열두 몸통 같은 상태 줄). 김기범 규칙: 버전은 지시가 있을 때만 — v0.4.5 태그·이미지는 지시 전에 나갔고 데스크톱 드래프트는 삭제. 로컬 `laf/redesign` 삭제, origin은 main 하나 | 다른 세션의 `laf/security-2026-09`(미병합 보안 수정)는 그대로 둠 |
| 2026-09-06 | bd7bba3 | **프로덕션급 W1 — 한국어 전면**: 조사 두 갈래(코드 감사, 화면 71개 결함 실측) 뒤 첫 작업. 갤러리 카드(승인·체크리스트·알림·거절·활동 보고) 전부 `t()`, 실패 문구 25곳·검증 문구 `t()`, 거절문은 모델용 영어와 화면용 한국어를 분리, `message-time`·루틴 시각에 `activeLocale`, 해요체 선언문 25→0, 에이전트/기술/동료/채널 → 봇/스킬/봇/대화, 사전 +97. 테이블 `t(variable)`와 카드 이름을 걷는 테스트(`import.meta.glob`은 bun에서 빈 배열이라 디렉터리를 읽음). 게이트 2008 | — |
| 2026-09-06 | 76a3d9a | **프로덕션급 W2 — 번들·모션·토큰·원시 요소**: `/sign` JS 3,163kB→671kB(−79%, `autoCodeSplitting`으로 22경로 전부 지연, vendor 4분할, `vendor-utils`가 없으면 `cn()` 때문에 markdown 1MB를 정적으로 끌어오던 함정 실측), 스트림다운·shiki·mermaid·katex는 트랜스크립트가 있을 때만. 움직임 줄이기: 기존 블록은 아무 요소도 안 잡던 셀렉터 → 속성 단위 캐치올(0.15s→0.001s 실측, 스피너는 2s로 감속). `ui/focus.ts`(focusRing 6종·selectedClass), `outline-hidden`으로 통일(강제 색상 모드), 선택=hairline+틴트(링은 포커스의 뜻), 비활성=채움 제거+not-allowed, `Button`이 `aria-pressed` 자동 반영. 라우터 `PageSkeleton` pending(300ms, 211ms 응답은 스켈레톤 없음 실측). `docs/laf/design-tokens.md` + 드리프트 테스트(허용목록 30파일 93건, 줄어들기만). 게이트 2016 | 중첩 `.dark`는 shadcn 기본값으로 떨어지는 잠재 함정을 문서에 기록 |
| 2026-09-06 | 1a33b0c | **프로덕션급 W3b — 봇·루틴·스킬·웰컴**: `lib/josa.ts`(`'김비서'을(를)` → 받침으로 을/를·이/가 결정, 사전이 `{josa}`를 받음), `layout/confirm-dialog.tsx`(⋯ 메뉴에서 열면 포커스가 `aria-hidden` 아래 남아 Tab으로 버튼에 못 닿던 것 실측 → 취소에 착지·순환), 파괴적 버튼 채움. 속도 컨트롤은 한 트랙의 세그먼트(`aria-pressed`), 얼굴 바꾸기 연필(호버·포커스)+메뉴, 로스터 800px에서 2열, 카드 120px 얼굴 띠, 로스터 전에는 `내 봇 0/5` 안 그림, 기억·스킬 카드 높이 보존. 루틴: 만들기 활성+누를 때 필드별 검증, 매일/특정 요일/N분마다(요일 칩은 가운데에서만; 월+수 → `dailyDays [1,3]` 실측), `clockLabel` 한국어 시각 한 종류(네이티브 time 입력 제거), 필드 순서, Switch 라벨, 삭제는 ⋯ 메뉴, 지금 실행 피드백, 다음/마지막 실행 표시, 생성은 URL 우측 패널(스킬과 같은 패턴). `layout/page-header.tsx`(세 페이지 공통), 스킬 `isError`·스켈레톤·슬러그 칩·sticky 저장, 웰컴 정렬. 게이트 2064(app 459/플로어 445) | Base UI가 모듈 로드 시 브라우저 여부를 한 번만 판정해 `bun test`에서 팝업 렌더 테스트가 순서에 따라 실패하는 하네스 결함 기록 |
| 2026-09-06 | 916bb81 | **프로덕션급 W3a — 대화·방·아바타·봇 화면·사이드바**(하위 에이전트 3개 병렬): 방 아바타 스택 기하(`channels/avatar-stack.ts`, 2명 대각·3명 삼각·4+ 2×2, 상자 밖으로 안 나감, 링 색은 prop). 실패한 턴: `laf_thread_runs.status='error'`는 이미 있었고 읽는 곳이 없었음 → `GET /api/channels/:id/failures`가 메시지별 사실 코드 6종을 답하고 트랜스크립트가 옆줄에 한국어+다시 시도(죽은 포트로 실측, 새로고침 생존). 빈 방 상태, 받는 봇 선택기 하나(`RosterStrip` 다중 선택), 참여자 팝오버 + `POST/DELETE …/participants`(스레드 보존·이름 재구성·거절 3종). 툴 줄 아이콘·구분점, 오른쪽 패널 한 왼쪽 선·모서리 분리·스켈레톤·포커스 링 8곳·`frame-bitmap.ts`(콜백형 `Uint8Array.from`이 39% 느렸던 것 실측), 날짜 구분선 600ms, 승인 화면 정중앙, 홈 정중앙(원래 아래로 32px 치우침). 사이드바 800px에서 64px 레일(35%→8%), 봇·방 행 한 레이아웃, `IconUsers`, 웹소켓 1회(StrictMode 재마운트를 참조 계수로). 게이트 2167(app 539/플로어 522) | 방의 실패 턴 재생은 `server/src/rooms/member-turn.ts`가 `threadId` 없이 시작·`run_id` NULL이라 미연결(기록); 닫힌 상세 패널 마운트는 `layout/detail-panel.tsx` 몫 |
| 2026-09-06 | 398b866 | **프로덕션급 W4 — 운영자·설정·연결·감사·i18n 게이트**: 감사 표 `켬` 열 → `Target`/대상; 서버 산문 `not in the current snapshot` → `laf:element_not_in_snapshot`(`auditFactCodes` 목록 + 코드마다 문장이 있어야 통과하는 테스트), `silenceOf`는 템플릿 리터럴이라 `t()`가 아예 없던 것 수정; `/admin/boundaries` 맨 문자열 5개(로더 2 포함); 플레이그라운드는 `sandbox="allow-scripts "`라 `localStorage`가 throw — 미리보기에 shim 대신 호스트로 오류를 넘겨 한국어 설명, Save/Publish/Delete pending. 연결 화면 마크 16종 인라인 SVG(브랜드 바탕+글자 또는 정확한 글리프), 타일을 제목 줄에 정렬(`ItemMedia`는 `ItemDescription`이 있을 때만 자기 정렬), `연결 안 됨` 15→0, 첫 섹션 간격 100→52px, 일반 카드 구분선, 알림 행 denied 상태(비활성 버튼+이유+바꿀 곳). 감사: 이벤트 종류·툴 id 라벨표(서버의 `auditEventTypes`·`COMPUTER_TOOLS`와 대조 테스트), 날짜별 그룹, 동일 행 접기(서명 기준이라 거절이 허용에 접히지 않음), 경계 행 한 모양·DSL 칩+주석·제거는 파괴적 메뉴, 권한·함수 버튼 42개 `aria-pressed`+개별 pending, `RowsSkeleton`/`LoadFailed`. **맨 영어 규칙**: `i18n-coverage`가 TS 파서로 118개 `.tsx`의 JSX 텍스트·placeholder·aria-label·title·alt를 걷음(잔여 1줄 허용목록, 심어 놓은 누출을 파일·줄로 잡는지 확인), `owner-vocabulary`가 연결 카탈로그 77문장을 읽음. W3b의 `ConfirmDialog`·`josa()` 채택. 리베이스 3회. 게이트 2203(app 575/플로어 557) | 남김: 컴포넌트·플러그인 거절 사유 영어 7곳(`components/store.ts`·`routes.ts`·`plugins/call.ts`), 설정 레일 행 40px vs 54px 토큰(후속 에이전트), `tool-boundary.tsx:33` 잔여 |
| 2026-09-07 | d972cd2 | **프로덕션급 후속**: 서버 거절 산문 11곳 → 사실 코드(`laf:component_unknown`·`_not_published`·`_withheld`, `function_unknown`·`_not_granted`, `tool_not_granted`, `skill_not_granted`, `action_failed`, `read_failed`, `tool_reported_error`, `one_shared_computer`) — 같은 문장이 감사 행·모델·대화 카드 세 독자에게 가던 것을 셋으로 분리(`tool-results.ko.ts`·`REFUSAL_SAID`·감사 `FACTS`), `isolation_loaded`의 note는 그릴 곳이 없었음 → 말이 있을 때만 그림, `/decision`이 `function`을 따로 반환. `h-row`(`--spacing-row`)로 설정·관리자 레일 행 40→54px 실측. `tool-boundary.tsx` 한 문장 `t()`+`josa()`, 맨 영어 허용목록 빈 집합. 게이트 2208(app 580) | `laf:skill_not_granted`는 아직 호출처 없음 |
| 2026-09-07 | a86a0c9 | 최종 검증 에이전트가 멈춘 뒤 남긴 수정 6파일 마무리(한국어 제목·플레이스홀더 `cafe24api.com`·얼굴 바꾸기 스크림 18:1/19.6:1). Hermes Agent 대조(`~/laf/docs/hermes-adoption-2026-09.md`) → 채택안 실행 시작 | — |
| 2026-09-07 | 6d4dd24 | **Hermes 채택 H4 — 루틴 제안**: 카탈로그 11(아침 브리핑·리뷰 감시·미답 문의·주간 정산·재고·예약·배송 지연·영업 상태·광고비·경쟁 가격·세금 일정), 연결이 `connected`일 때만 제안, 봇 없으면 없음, 키·이름 중복 제거, 거절 래치 표(0034), 최대 5, 수락은 기존 생성 경로. 실측: 카드 5장, 만들기→루틴 행, 다음에→새로고침 후에도 사라짐. 게이트 server 1433·app 591 | 공고·입찰은 카탈로그 사이트가 아니라 제외 |
| 2026-09-07 | 456de2f | **H2 — 기억·툴 결과**: `looksLikeAnInstruction`(역할 표식·이전 무시·2인칭 지시·한국어 명령 어미·URL 가져오기·툴 호출 문법) → `laf:memory_looks_like_instruction`, 봇당 2,200자 → `laf:memory_full`, 실행당 스냅샷 고정. 스필오버는 copilot 미들웨어에서(agent-bot은 서버 주소를 모른다는 결정 유지): `.results/<id>.txt`를 봇 자격으로 컴퓨터에 쓰고 다음 실행부터 1,500자 미리보기+경로, 턴 예산 20,000. 게이트 server 1435·root 86 | — |
| 2026-09-07 | c462361 | **H1 — 프롬프트**: 루틴 `[SILENT]`(전체/첫줄/끝줄, `조용히`, 사람 기다리는 실행은 침묵 불가; 대화·알림 없음, `routine.ran silent:true`), "툴 결과는 데이터" 문단, 시각 줄을 맨 끝으로 + `eval:cache` A/B **실측**(glm-5.3-flash, 10턴 150k 토큰: 캐시 1.4% → 7.5%; 시각을 트랜스크립트 뒤에 두면 18.8% — 미출하), 스킬 인덱스(부여된 것만, ≤1,400 토큰)+`skill_view` 툴(`skill.viewed`), CLAUDE.md Footprint Ladder. 게이트 1439/580/132/89 | 시각을 트랜스크립트 뒤로 옮기는 안은 후속 검토 |
| 2026-09-07 | b457d28 | **H3 — 허락 층·방·봇 간 호출**: 세 번째 눈금 "이 대화 동안"(`computer_standing_approvals`에 `thread_id`·`expires_at`, 0035; 그 대화에서만·24h·대화 종료 시 회수; 카드 세 번째 버튼은 질문이 대화를 실을 때만; 경계 화면에 눈금·만료 표시; 감사 `tier/thread/expiresAt`; `deny`는 불변). 방 발언 규칙 `rooms/turn-taking.ts`(0라운드 @멘션/칩 — 한국어 조사·최장 이름 우선·호격, 이후 다른 봇이 부른 봇만, 3라운드·10메시지, 회전; `room.member_turn` 감사), 방 실패 턴 연결(멤버 행에 `threadId`·`runId`, 사람 메시지 `run_id=turnId` → `/failures`가 방에도). 위임: `laf:ask_in_delegated_turn`(상시 허용은 존중), `laf:delegation_too_deep`(깊이 1, 403), 피호출 봇은 툴 없이. 게이트 server 1532·app 593·root 97 | 브라우저 실측은 안 함 |
| 2026-09-07 | acfbdfc | **H5 — 연결 툴 지연 노출**: `mcp__*` 24개(드라이브 4·시트 4·지메일 4·캘린더 2·비즈니스 프로필 3·카페24 5·알림톡 2)를 `tool_search/tool_describe/tool_call` 브리지 뒤로; 컴퓨터 14·self 3·`send_message`·`ask_coworker`·브리지는 절대 지연 안 함(이름으로 단언). 실측 스키마 23,175→13,166B(−43%; Hermes −56%보다 작은 이유는 한국어 설명 3바이트/자). 브리지는 실행 안에서 답하고 `tool_call`을 실제 이름·인자·id로 바꿔 settle·감사·가드는 그대로. H2의 턴 예산과 같은 루프에서 처리(리베이스 해결). `eval:model` deferral 팔 + 브리지로 메일 보내기 시나리오. 게이트 server 1540·app 593·root 132 | 프롬프트 토큰은 키 없는 워크트리라 미측정 |
| 2026-09-07 | 85f4707 | **출시 1-A — 문이 열린다**: laf-control 3b82493 `core/customer-env.ts`가 개통·`env push`에서 `AUTH_PROVIDERS=laf`·`LAF_OIDC_ISSUER/CLIENT_ID`(공개 PKCE 클라이언트, fqdn 결정적, 스택 전에 auth VM `clients.json`에 등록)·`INITIAL_ADMIN_EMAILS`/`SIGN_IN_ALLOWED_EMAILS`·`BETTER_AUTH_SECRET`(VM에서 생성)·`OPENAI_*`/`BOT_MODEL`(`LAF_FLEET_*`, fail-closed)을 한 번의 `umask 077` 쓰기로 실음, `--tag edge`, `laf member`가 자동 push, `docs/rehearsal-provision.md`. openbot: `GET /api/auth/providers` + `/sign`이 배포의 목록을 먼저 읽고 구운 목록은 폴백(실측: 빈 이미지+`laf` 서버 → 버튼 3개). 게이트 server 1438·app 599; laf-control 272(플로어 263) | 함대 API `api/auth.ts`가 `GOOGLE_OAUTH_CLIENT_ID`를 읽어 노트북 `.env`의 커넥터 키와 충돌(로컬 통합 테스트만 영향) — 후속 |
| 2026-09-07 | 72638ff | **출시 1-B — 보안 가지 통합**: 다른 세션의 7개 브랜치를 골라 16커밋으로 착지(중복 3쌍 제거) — `describeFailure` 기반, 봇 id 형태 단일 정의(`isBotId`)와 경로 금지(라우트·사이트 본문·클라이언트·컴퓨터), CopilotKit 런타임 `requireUser`, 대화는 주인만(401/404 사실 코드), 실패한 쿼리가 쿼리를 되돌려주지 않음, 리졸버 범위·철자, 봇 브라우저의 사설 호스트 거부, `.env.example` 옵트인 기본 off, 상태 변경 `/api`와 두 업그레이드에 origin 검사, 라이브 화면 `streamBotAccess`, `actorMayDriveBot`, 시연 읽기·폐기는 요청자 범위, 승인 라우트 id 검사, 스냅샷 비밀값 마스킹·오래된 캐시 추적·`laf:key_is_text`·비밀 대상 해석·지문에 element·호스트 정규화·쿼리 없는 추적. 마이그레이션 없음. 게이트 server 1677·app 601·agent-computer 136·root 133 | `routines/service.ts` 13줄 헝크는 3-B 뒤로 미룸 |
| 2026-09-07 | 264da39, ab53b3c | **CI — `:edge`는 main이다**: 리허설이 `:edge`가 main보다 12일(로그인 변경 2건) 뒤처진 것을 발견 — GHCR의 어떤 이미지도 새 고객을 로그인시킬 수 없었음. `images.yml`이 main 푸시마다 `:edge`를 발행(문서만 바뀌면 건너뜀). `release.yml`의 `dtolnay/rust-toolchain` 핀은 그 저장소가 브랜치를 force-push해 zizmor impostor-commit으로 8월 5일부터 매번 실패 → `actions-rust-lang/setup-rust-toolchain` v1.17.0 태그 커밋으로 교체(cache: false) | 워크플로 dispatch·`gh run view`가 auto 모드 분류기에 막힘 — 트리거를 코드로 바꿔 해결 |
| 2026-09-07 | fa5c3f9 | **출시 2-C — 공고·지원사업**: 플러그인 `public-data`(배포 키, 읽기 전용) — `search_bids`(나라장터 용역 입찰공고)·`search_support_programs`(기업마당). 감시 루틴은 기존 `manage_routine`으로(사다리 첫 단). 결과는 사실만(제목·기관·기한·금액·url). 실측: 나라장터 "청소"/7일 21건 1.1s, 기업마당 "소상공인" 225건 77ms. 실제 서버로 부팅해 확인하다 게이트가 놓친 배선 버그 2건 발견·수정(런타임이 `createApp`에 안 넘어감, 키 조회가 공개 URL 연결 설정에 묶임). 게이트 server 1706 | VM에서 하루 도는 `[SILENT]` 확인은 리허설에서 |
| 2026-09-07 | c14a013 | **출시 3-B — 끝내지 못한 실행은 말한다**: 부팅 시 `unknown`으로 정리된 실행을 `reportInterruptedRuns`가 루틴 대화 표시 + `run.failed` 아웃박스(`laf:turn_interrupted`, 소켓·웹훅만, 알림톡 절대 안 감)로; 루틴 `RUN_ERROR`·기한 초과도 같은 길(`routine.ran` → `withOutboxWatch`). `/failures`가 `unknown` 실행을 보고, 다시 시도는 사람의 메시지 아래에서만. 따라잡기 유예 = 주기의 절반(2분~2시간), `routine.caught_up`/`routine.skipped_missed`. 재시작을 건넌 승인 카드는 사라지고 다음 턴에 다시 묻는다(허용으로 문서화). 실측: 느린 가짜 AG-UI → `kill -9` → 부팅 로그 → 아웃박스 → 웹훅 본문 → 앱 "끝내지 못했습니다. 도중에 서버가 다시 시작됐습니다." 게이트 server 1685 | 공유 55432 컨테이너가 다른 세션의 DROP DATABASE로 4번 죽음 — 격리 Postgres에서 게이트 |
| 2026-09-07 | 13c19c3 | **출시 2-A — 첫 과업 칩**: 어떤 채널에도 메시지가 없는 봇의 `/channel/new` 소개 카드 아래에 칩 6개(오늘 날짜·공휴일, 가게 소개 문구, 오픈 체크리스트, 환불 답장 초안, 사이트 연결하기(연결 없을 때), 매일 아침 7:30 보고받기 → `POST /api/routines`). 연결된 사이트·계정이 있으면 그 카드의 첫 문장이 앞에, 역할(프리셋·직접 입력)이 있으면 그 패턴이 앞에. 브라우저 실측: 한 번 누르면 채널 생성·문장 전송·`laf:first-task-pressed` 1회; 이력 있는 봇은 칩 없음. 36개 테스트, 플로어 app 617 | 리베이스 때 `i18n-ko.ts` 충돌을 양쪽 보존으로 풀다 주석 여는 줄이 빠짐 — 병합 뒤 손으로 복구 |
| 2026-09-07 | laf-control 6f63b31 | **출시 3-A — 함대 감시·알림**: `laf watch`(1회 / `--loop [--every 300]` / `--quiet` / `--install-cron` / `--cert-days` / `--job-minutes`), 신호 = health·cert·offsite·jobs(종류별 최신 작업; 실패·중단·30분 이상 실행). 전이만 `LAF_ALERT_WEBHOOK_URL`로(Slack·Discord 호환 JSON + `fleet{customer,origin,signal,from,to,value,at}`), 복구도 한 번("작업 복구 — … (1분 만에)"), 사이 반복 없음. `laf backend api`가 CLI 번들을 실어 박스의 cron이 돌림, `GET /api/fleet/alerts`. 로컬 리스너로 실측. 테스트 324(플로어 313) | 노트북 `.env`의 `GOOGLE_OAUTH_CLIENT_ID`가 함대 API 로그인 키와 충돌 — 테스트는 그 쌍을 비우고 실행 |
| 2026-09-07 | c8c0172 + laf-control 20bc1e5 | **출시 3-C — 복원 리허설·30일 보관**: openbot `scripts/restore.sh`(`--dry-run`, `--fresh`, 표 수·동등 비교 요약만). laf-control `laf restore <slug> [--from 날짜] [--dry-run]` 5단계 작업(버킷에서 덤프 고르기 → VM 점검 → 1객체·1시간 읽기 문을 열어 VM이 직접 내려받음(URL은 ssh stdin → root 파일 → `curl -K`, argv·노트·원장에 없음) → `openbot_restore`로 복원 → `customer.restore` 감사). `laf offsite lifecycle <slug>`. 30일 규칙(객체·이전 버전) 한 표에서 OCI·S3 양쪽에 개통 시 적용, 내용으로 멱등, 되읽어 확인. 실측: 개발 DB 복원 33표/33동등/1s, 스왑 1.8s, `sajuhook` 버킷 규칙 30일 확인. laf-control 병합 0ff5c30(테스트 355, 플로어 313) | `rehearsal1`이 아직 없어 실 고객 드릴은 리허설로 |
| 2026-09-07 | 835d9c0 | **출시 4-B — 약관·처리방침·동의·화면 열람**: `/legal/terms`·`/legal/privacy`(공개, `/terms`·`/privacy` 리다이렉트), 1인 사업자용 한국어 초안(`app/src/legal/*.md`, 초안 표시는 HTML 주석, 버전 줄·연락처), `/sign`·설정 푸터 링크. `0036_consent`(`consented_at`, `consent_version`), `POST /api/me/consent`는 환영 화면의 다음 버튼이 찍음, `/api/me`가 동의 버전 vs 현재를 보내 불일치면 `/consent`에서 다시 물음, 내보내기에 포함. `computer.screen_viewed`는 라이브 소켓당 1회·녹화 읽기당 1회(주인 포함), 관리자 감사에 "화면을 봄 · 자기 봇 · 실시간/시연". 실측: 환영→204→`onboarded_at` 전에 도장; 버전 불일치→`/consent`→도장 갱신; 소켓 1프레임→1행, 읽기 3회→1행. 발견·수정: `recordAuditEvent`가 저장 오류를 삼키지 않음. 게이트 server 1739·app 650 | 리베이스 충돌 3파일(2-C 배선·i18n)은 해결 에이전트가 처리 |
| 2026-09-07 | 3da44eb | **출시 3-D — 로그 위생**: `shared/log.ts` — 한 줄 한 JSON `{level, at, svc, event, …사실}`, 오류는 `describeFailure`(스택 없음), 비밀 이름 키는 `[redacted]`, 문자열 안의 `sk-…`/`Bearer …`/JWT/`user:pw@` 절단. `boot{version(IMAGE_TAG, compose가 넘김), model, tools, port}`, `shutdown{reason}`, `crashed`가 Bun의 날 덤프를 대체. `describeFailure`는 `shared/failure-text.ts`로 이동해 OpenAI 클라이언트 오류를 `provider_rate_limited`/`provider_refused`/`provider_unavailable`/`reply_unusable`로 분류. 카나리 테스트(실제 서버+agent-bot 서브프로세스, 가짜 제공자, 2턴): 키·비밀번호 메시지·429 본문 부재, 모든 줄 JSON. 전에 새던 것(실측): agent-bot이 제공자 본문·헤더·스택 통째, `rooms/service.ts`가 Drizzle SQL+메시지 배열, 부팅 실패가 값 포함 `update "laf_thread_runs"`. `tests/log-discipline.test.ts` 천장. 플로어 1636/617/131/142 | 리베이스 3회(3-B·4-B 뒤) — 다른 가지가 들여온 `console` 6곳도 변환(마지막은 4-B의 `screen-view.ts`) |
| 2026-09-07 | 0f5477e | **출시 2-B — 직무 4종 실측** (UI로 만든 봇, glm-5.3-flash, 워크트리 실제 스택): (1) 주문 목록 읽기(픽스처) 첫 반응 7.7s·완료 17.4s(카드를 그리면 51.9s), 승인 0 · (2) 알림톡(가짜 솔라피) 첫 반응 4.7s·완료 33.8s, 승인 1(19.5s) — 수정 전엔 승인 2회 낭비 뒤 거절 · (3) 기업마당 요약 304s 포기 — 컨테이너에서 사이트 도달 불가(호스트 0.7s) · (4) 대화로 주간 루틴 만들고 즉시 실행 — 생성 14~16s, 실행 11.3s. 수정: 내비게이션 타임아웃이 "요소가 사라짐"으로 보고되던 것 → `laf:page_timeout` 504와 러너 코드 번역; `alimtalk_send` 스키마가 템플릿별 빈칸과 거절 문장을 이름으로; 픽스처 페이지+테스트; eval 시나리오 `send-alimtalk-with-the-blanks-named`. 게이트 2556 | 남은 사실 7건(멈춘 사이트가 봇 브라우저를 영구히 막음, 파트너 권한이 연결 시점의 봇에만, 인자 검증 전 승인, 화면 창의 영어 오류, 채널 열고 0.5s 안의 키 입력 이탈, 첫 전송 +6.2s, 컨테이너→기업마당 불가) → 후속 작업 착수 |
| 2026-09-07 | 4732a34 | **출시 4-A — 도움말·문의·의견**: `/help`(한국어 마크다운 `app/src/help/guide.md` 다섯 절, 굵은 버튼 이름은 전부 `ko` 표에 있는지 테스트), 명단 푸터의 `?`, 설정의 도움말 절, 법률 문서 링크. 문의·의견 대화상자(설정·도움말): 2,000자, "지금 화면을 같이 보냄"은 기본 꺼짐이고 경로+마지막 실패 코드만(`lib/support/last-failure.ts`, 실패 줄이 기록). `POST /api/support/feedback` → `laf_feedback`(`0037_feedback`), 아웃박스 `support.feedback`은 새 `LAF_ALERT_WEBHOOK_URL` 문으로만(함대의 `{text, content, feedback}` 모양; 사람에게 가는 문과 `/api/me/notifications`는 못 봄), 감사 `support.feedback_sent`(본문 없음). 실측: 웹훅 본문·DB 행·감사 페이로드·빈 알림 목록. 게이트 2747(플로어 server 1721·app 653) | `korean-sites` 브라우저 테스트 2건이 부하에서 한 번 흔들림(단독 14/14) |
| 2026-09-07 | b1ab490 | **서버 부팅 경주 — `eventsource`**: 3-D 카나리가 CI에서 실패한 실제 원인은 DB URL이 아니라 서버가 import 단계에서 죽는 것 — `@ag-ui/mcp-apps-middleware`(CJS) → SDK `client/sse.js` → `require("eventsource")`를 Bun이 ESM 빌드로 풀어 런타임 ESM 그래프와 경주(`require() async module … unsupported`). `oven/bun:1.3.14`·2 CPU·6개 동시 부팅에서 **7/30 크래시 → 0/30**. 수정: `server/src/index.ts`는 `await import("eventsource")` 뒤 `await import("./main")` 두 줄(서버 본문은 `main.ts`로), `eventsource`를 런타임 의존성으로. 카나리는 하위 프로세스 env를 명시하고 죽으면 exit code+stderr로 즉시 실패, 이름 있는 테스트. 증명: `env -i … bun test tests/log-hygiene.integration.test.ts` 4 pass | 4-A 뒤 리베이스에서 `index.ts` 이동 충돌 — main의 본문을 `main.ts`로, 수정의 진입 파일을 `index.ts`로 |
| 2026-09-07 | 6ce513f | **출시 2-B 후속 — 실측이 남긴 7건**: (1) 멈춘 사이트 → 고침: 기한을 넘긴 내비게이션은 탭(`recycled: "page"`) 또는 브라우저를 버리고, 3s 유예를 넘긴 close는 CDP로 기록한 pid를 죽임, 504 `laf:page_timeout`. 픽스처 `/hang` 2s 기한: 타임아웃 2.06s, 다음 내비게이션 0.54s, 멈춘 중 stop 0.27s·reset 0.30s(전엔 30.1s/502, stop >120s). (2) 파트너 권한 → 고침: `PartnerRuntime.grantTo/offerTo`, 생성 훅이 연결된 파트너 툴을 새 봇에 부여. (3) 인자 검증 전 승인 → 고침: `call.ts`가 스키마 `required`/`enum`(`laf:tool_arguments_invalid`)과 `validateArgs`를 정책·승인 전에, 빈칸 빠진 `alimtalk_send`는 카드 없이 거절. (4) 화면 창 영어 오류 → 고침: 코드 11개를 한 표에서 `t()`. (5) 0.5s 키 입력 이탈 → 고침: 열 때 포커스 없음 + 렌더마다 재생성되는 트리거/`onChange`(테스트가 옛 코드에서 `녕하세요안` 재현). (6) 첫 전송 ~6s → 사실: Enter→실행 요청 165ms(새 채널)/244~251ms, 채널 생성 31~47ms, 나머지는 모델(RUN_FINISHED 4.6~7.5s). (7) 기업마당 → 사실: 컨테이너 curl 200/0.8s, 단독 Chromium 9/9, 컴퓨터 경유 2봇 중 1이 두 번째 로드에서 멈춤 — 이제 (1)로 복구됨. Biome 경고 6건 정리. 게이트 2743, 플로어 1725/653/142/151 | `scripts/test-ci.ts` 플로어 충돌은 더 높은 쪽으로 |
| 2026-09-07 | laf-control 21b8298 | **함대 `reconcile` — auth를 "없음"으로 보던 것**: 원인은 범위가 아니라 `auth`와 `entry`가 같은 인스턴스(140.238.1.112)를 가리키는 두 행인데 클라우드 인스턴스를 첫 행에만 묶던 것 → 행마다 자기 인스턴스를 찾음. 범위는 정책 ∪ 명부 ∪ `--region`(`reconcileScope`, 출처 표시). 실행: 불일치 0. **발견**: 춘천(`ap-chuncheon-1`)은 테넌시가 구독하지 않아 `NotAuthenticated` — 용량 폴백은 오너가 구독하기 전까지 같은 이유로 실패. 테스트 359(플로어 348) | 리허설 2의 VM은 슬러그 `hb25c3r5fe.agent.laf-co.com`으로 개통 PASS(플랜 §6) |
| 2026-09-07 | (측정만) | **출시 3-E — 성능 수치** (국내 회선 → 서울 A1, `:edge` fa5c3f9, 로그인 없이): curl TTFB 중앙값 `/` 29ms · `/sign` 28ms · providers 28ms; 첫 화면 preload gzip 266kB(원본 911kB); 헤드리스 Chromium `/sign` DOMContentLoaded 123ms · FCP 208ms · 전송 345kB(목표 3s). VM: 컨테이너 합 401MiB(서버 201·컴퓨터 95·postgres 64), 가용 4,769MB, 디스크 13/45G, load 0.27; 서버 시작→listen 3.05~3.80s; Postgres 연결 16·9.4MB·36표. 재시작: 바깥에서 본 단절 5.9s, 명령→200 16.6s. **틀린 것**: 공개 `/health`가 Caddy의 SPA 폴백(index.html 1,790B)이라 API가 죽은 5.9s 동안도 200 — `laf watch`와 리허설 2의 "health 200"이 정적 파일을 읽고 있었음(`/api/health`는 404); 배포 이미지가 main보다 26커밋 뒤(SIGTERM 핸들러 없음 → stop마다 10s 뒤 137, `eventsource` 경주로 3번 중 1번 부팅 실패를 `unless-stopped`가 살림). 첫 토큰 p50/p90은 세션이 있어야 측정 | 후속 착수: `/health`를 API로 프록시(openbot) + 감시가 JSON `status`만 건강으로(laf-control), 그 뒤 `laf upgrade rehearsal1`로 업그레이드 리허설 |
| 2026-09-07 | 78ddc6b + laf-control 69929dc | **건강은 상태 코드가 아니라 본문이 말한다**: 3-E가 찾은 것 — 공개 `/health`가 Caddy의 SPA 폴백이라 API가 죽어도 200. 제품: Caddy가 `handle /health { reverse_proxy server:3001 }`을 폴백보다 먼저, `createApp`이 같은 라우트 인스턴스를 `/health`와 `/api/health`에(캐시 하나·프로브 한 벌), web 컨테이너 헬스체크는 Caddy admin API 대신 `localhost:2021`로 `/health`를 실제로 물음. 실측: 정상 200 `{"status":"ok","checks":{database,agentBot,computer}}`, API 정지 502 빈 본문(옛 헬스체크는 0, 새 것은 1), Postgres 정지 503 `degraded`+`database:down`. 함대: ok는 200 **그리고** JSON `status:"ok"`, `degraded`는 별도 상태(◐·저하 알림·exit 1), HTML·비JSON·status 없음은 `unreadable` 실패. 실측: apex 고객 규칙에서 `HTTP 404 · unreadable (empty)`, 죽은 sajuhook `응답 없음`, 로컬 Caddy의 빈 200도 빨강. 게이트 openbot 2796·laf-control 382 | 앱 재빌드가 이 도커 VM에서 OOM — Caddyfile만 얹어 검증, `/srv`는 이 커밋이 안 건드림 |
| 2026-09-07 | (실측만) | **업그레이드 리허설** (시험 VM `hb25c3r5fe`, fa5c3f9 → 78ddc6b, `laf upgrade`): 136.0s에 5개 컨테이너 healthy(풀 86.1s·재생성 36.6s·헬스 대기 12.6s). 바깥에서 본 단절 — `/` 13.6s(전부 connection refused: `compose up -d`가 Caddy 컨테이너를 다시 만들어 리스너가 사라짐), `/health` 16.7s(13.1s 거부 + 정직한 502 3.0s). `/health` 전: 200 text/html 1,790B, `/api/health` 404 → 후: 200 JSON 74B `{"status":"ok","checks":{database,agentBot,computer}}`, API 정지 시 502 0B, 딥링크는 여전히 SPA, providers 그대로. SIGTERM: `docker stop` 10.5s→**0.22s**, exit 137→**0**, 재시작 0.35s, 바깥 502 창 5.9s→3.0s(4회 모두), 6번 부팅 0 크래시. 함대: `status` `● HTTP 200 · ok`, `watch` exit 0 → 서버 정지 시 `○! HTTP 502 · unreadable (empty)` exit 1 → 복구 알림 | **깨진 것**: `laf watch`가 `relation "watch_states" does not exist`로 죽음 — 마이그레이션 0008이 AWS 명부에 한 번도 적용되지 않았음(문서에는 있음). 손으로 적용. cron이 깔려 있었다면 매 패스마다 죽어 아무도 보고 있지 않았을 것. 그리고 옛 이미지 1.889GB가 남음(디스크 29%→34%) → 후속 착수 |
| 2026-09-07 | laf-control b80757d | **업그레이드 리허설이 찾은 것 세 가지**: ① `core/migrations.ts` — `laf backend up`·`api`가 명부의 밀린 마이그레이션을 ssh→`psql`로 직접 적용하고(SQL은 파일로, `pipefail`·`ON_ERROR_STOP=1`) 개수를 보고하며 **번들을 밀거나 유닛을 재시작하기 전에** 실패하면 던짐. 원장은 drizzle-kit의 것이라 `db:migrate`와 이중 적용 없음. `watch`·`status`는 빠진 마이그레이션 이름을 말하며 exit 69. 증명: 0008 없는 명부에서 "마이그레이션 1개 적용", 실서버는 "0개 — 이미 0008까지". ② 정문 단절: `web`을 재시작 묶음에서 빼 마지막에, 이미지가 바뀐 경우에만 재생성 → 실측 `/` **11.9s → 0.5s**, 이미지가 그대로면 **0s**; `laf upgrade`가 시작 전에 비용을 알림. ③ 마지막 단계에 `docker image prune -f` → **1.889GB 회수**, 디스크 34%→30%, 두 번째 실행 0B. 테스트 400(플로어 388) | |
| 2026-09-07 | 4a81a67 | **Hermes 대조 II**: 최근 커밋(태그 이후 5,355개)을 다시 읽고 열 영역을 판정 — LAF 앞섬 2(승인·문서), Hermes 앞섬 4(루프 측정·스킬·포장·경로 커버리지), 비김 2, 갈림 2. 8월 채택 후보 11개는 전부 착지했고, 루틴 따라잡기 유예는 그들의 공식·상수와 같음(코드가 출처를 밝힘). 다음 채택 후보: 루틴 메모장(잡별 커서), 실패 사건 묶기, 자동 리뷰의 주입 방어. 문서는 저장소 밖 `~/laf/docs/hermes-comparison-2026-09-07.md` | 읽다가 찾은 두 결함을 이 커밋이 고침: `evals/deferral.ts`가 어떤 스크립트에서도 닿지 않았음(`eval:deferral` 추가), `CLAUDE.md`가 아직 pgvector라 적음(0024가 확장을 드롭, compose는 `postgres:17`) |
| 2026-09-07 | b1c308b | **설치형 앱 — dmg와 exe**: `release.yml`에 태그 없는 빌드 전용 경로 추가(main 푸시마다, 문서만이면 건너뜀) — 릴리스를 만들지도 넓히지도 않고 워크플로 아티팩트로만 남김(`darwin-universal-dmg`, `windows-x64-nsis`, 90일). 빌드 전용은 업데이터 서명 비밀 없이도 돌게 했고, **자기 자신을 0.4.4로 교체하던 버그**(빌드 전용 설치본에 업데이터가 살아 있었음)를 업데이터 제거로 고침. 셸: 첫 실행은 정문, 그 뒤에는 마지막으로 머문 `<이름>.agent.laf-co.com`을 `shell.json`에 기억해 다시 엶(권한 목록으로 재검증 — `evil-agent.laf-co.com`은 거부하고 정문으로). `origin()`이 창의 실제 origin이라 딥링크·알림이 배포 기준으로 풀림. macOS 15에서는 종료 시 `ExitRequested`·`CloseRequested`가 안 와 `Focused(false)`에 걺. 로컬 빌드 실측: `LAF Agent_0.2.0_aarch64.dmg` 2.6MB, 애드혹 서명, arm64(이 맥의 Rust에 x86_64 std 없음 — CI는 둘 다). `docs/laf/installing.md` | 코드 서명은 오너 결정(D4) — 지금은 게이트키퍼·스마트스크린 경고가 뜨고 문서가 그 대처를 적음 |
| 2026-09-10 | 1ecd51f | **비공개 저장소에 맞춘 CI**: 오너 결정으로 저장소가 비공개로 간다. 조직은 GitHub Free(월 2,000분, macOS ×10, Windows ×2, arm64 러너는 과금) — 9월 1~10일 실측 Linux 627·Windows 111·ARM 107·macOS 83분 = 가중 약 1,780분, 그대로면 11일 만에 한도. `release.yml` 빌드 전용 경로는 `desktop/**`·워크플로 파일이 바뀔 때와 dispatch만(태그 경로 불변, macOS universal 유지); `images.yml`은 코드 푸시마다 `:edge` 유지 + ref별 concurrency cancel-in-progress(태그 제외); `ci.yml`은 PR과 `laf/**`만, main·태그의 검사는 `images.yml` 안에서 한 번. arm64는 `ubuntu-24.04-arm` 유지(비공개 표준 러너, $0.005/분). 추정 월 1,600~1,700분(태그 3회 기준, 태그당 ≈150). zizmor 1.30.1 clean. `docs/laf/deploying.md` "비공개 저장소의 CI 비용" | `desktop/README.md`의 "매 푸시" 문구를 병합 때 고침 |
| 2026-09-10 | 3b1867a + laf-control 323017f | **비공개가 되는 날 VM이 필요한 것**: (1) 이미지 pull — 고객 VM이 익명으로 받고 있었음(실측 200). laf-control이 `LAF_FLEET_GHCR_USER/TOKEN`(read:packages만)을 개통·`env push`에서 ssh stdin → `/root/.laf-ghcr-door` → `docker login --password-stdin`으로 심고 문을 지움; 자격증명은 `/root/.docker/config.json`(600)에만, `.env`·노트·원장·argv에 없음. 토큰 없을 때는 익명 manifest HEAD로 먼저 확인해 공개면 진행(메모), 비공개면 VM을 건드리기 전에 한국어 한 문장으로 거절. 테스트 19(420, 플로어 407). (2) 소스 clone — VM이 `git clone`으로 compose를 갖고 있었음(비공개면 끊기고, 소스가 고객 기계에 통째로). 다섯 번째 이미지 `openbot-deploy:<tag>`(`FROM scratch`, 28KB, amd64+arm64, `/deploy/`에 compose·`scripts/upgrade.sh`·`scripts/restore.sh`·`.env.example`·`VERSION`) — `docker create`/`cp`로 꺼내며 `.git` 불필요. 로컬 증명: 다섯 파일 `diff -r` 동일, `compose config` 파싱 | 함대 쪽 추출·마이그레이션(git 시대 VM은 `.git`을 옆으로 옮김)은 진행 중 |
| 2026-09-10 | laf-control 7072f6d · 저장소 PRIVATE | **비공개 전환 실행**: 함대가 VM에서 `git clone/pull` 대신 배포 묶음을 `docker create`/`cp`로 꺼냄(`core/deploy-bundle.ts`; 업그레이드 단계 "배포 묶음 받기 → 이미지 당기기 → 재기동(정문 제외) → 건강 대기 → 정문 교체(이미지 바뀐 경우만) → 바깥 검증 → 버전 기록 → 정리"; `status`가 `edge · 묶음 3b1867a`). 시험 VM 실측: `laf upgrade rehearsal1` — `.git`은 `~/openbot.git-20260910`로 옮김, `VERSION`=3b1867a/edge, `.env` 그대로, 5 컨테이너 healthy, `/health` ok, 옛 이미지 1.89GB 회수. 남아 있던 옛 소스 25항목(~12MB)은 손으로 같은 보관 폴더의 `checkout/`으로 옮김(compose는 호스트 마운트 없음 — 확인) → 함대 마이그레이션이 통째로 옮기도록 후속. 그 뒤 `gh repo edit --visibility private`: openbot **PRIVATE**, 이미지는 아직 공개(익명 manifest 200)라 VM은 계속 받음. 테스트 444(플로어 430) | 이미지를 비공개로 돌리는 것은 오너가 `read:packages` 토큰을 만든 뒤(`LAF_FLEET_GHCR_TOKEN`); 비공개 저장소의 첫 CI(arm64 러너 과금)는 다음 코드 푸시에서 확인 |
| 2026-09-10 | 51221e6 | **감사 1파동 W1-h — 나쁜 날의 상자**: (감사 A5·A6) `web`이 아무것도 `depends_on` 하지 않음 — 마이그레이션이 틀린 비밀번호로 실패해도 `/` 200, `/health` 502(Caddy; 사유 있는 503은 `handle_errors` 필요 — 후속). `upgrade.sh`의 롤백 문구가 안전한 형태(`restore.sh <dump> --replace` = 옆에 복원·diff·스왑, 실측 22s에 healthy), 실패한 pull은 exit 1에 다섯 컨테이너 그대로·`/health` 200, `.env` sha 동일; `tests/upgrade-script.test.ts`(가짜 docker, 8 케이스). 백업은 DB만(함대 스크립트가 `pg_dump | gzip` 비암호화라 은행·홈택스 쿠키가 든 프로필은 버킷에 안 올림), 새 VM 복원은 봇별 재로그인 — `restore.sh --replace`가 `laf_site_connections`를 `needs_login`으로 표시(실측 1건). 한도: server 1536m·postgres 1g(`oom_score_adj -500`, shm 256m)·agent-bot/migrate 512m·web 256m·브라우저 3g, 로그 10m×5, Postgres 256MB/768MB/8MB(`SHOW`로 되읽음); agent-bot 할당기를 256MB에서 죽여도 서비스 healthy. 삭제 잔여 2건은 `data-lifecycle.md` §3에 기록(보존이 끝냄). 루트 테스트 174 | Dependabot 경고가 켜져 첫 경고 1건(moderate) 도착 |
| 2026-09-10 | cc511e9 | **감사 1파동 W1-a — 봇 id는 그 봇을 몰 수 있는 사람에게만 문을 연다**: (감사 A8 S1 ×2, S2 ×3, A6 S2) 라우트 셀 36개 + 소켓 닫음 — `/api/computers/:botId/*` 27개, `approvals/:botId*` 2개, 컴포넌트 3개(`for-agent`·`decision`·`call`의 본문 id), 루틴 `create`(트리거 토큰은 몰 수 있는 봇에만; 없는 봇은 FK-500이 아니라 404), `agents/:agentId/memories` 3개(`store.get`의 null을 무시해 B가 A의 비공개 봇에 200). 전부 하나의 `requireBotAccess`/`mayDriveBot`: 모양 → 소유 → 404 `laf:bot_not_found`. 라이브 화면은 가시성이 아니라 `roleRepository.botOwner`로. 세션 가드 "역할 없음" 분기 테스트 2개(변이에서 실패 확인). **권한 행렬 테스트** `authorization-matrix.integration.test.ts`: `app.routes`의 142 라우트 × 4 주체 = 568셀, 실제 스토어, 허용 목록 명시(B 30·A 61·admin 75). 게이트 server 1829 | 남은 것: `GET /api/plugins/for/:agentId`·`skills/:slug/view`(plugins는 W1-c 소유 → 후속), 관리자 자격증명 폐기의 미지 id 500(`app.ts` 인라인 라우트, `CredentialUnavailableError` 미매핑 → W1-e의 onError) |
| 2026-09-10 | 354e80f | **감사 1파동 W1-c — 연결이 사실을 말한다**: (감사 A9 F1~F7, §1) F1 부여 규칙 하나 — `plugins/servers.ts` `grantConnectionTo`를 연결 시(`offerToolsTo`)와 봇 생성 훅(`offerConnectionsTo`) 둘 다 부름; 실제 라우트로 증명(시트 연결 → `POST /api/agents` → `/for/:id` 툴 4개, 먼저 만든 봇과 동일, 재부여 0행). F2/F3 `connection-health.ts judgeHealth(reason)` 26행 표 — 토큰 엔드포인트 503/429는 `vendor_down`(ok, 다음 호출은 벤더로), 교환 성공 뒤 API 401은 `refresh_failed`/재연결(다음 호출 교환 0회 거부). F4 다른 봇의 로그인 벽은 `null`, 자기 봇만 `needsLogin`(별도 커밋). F5 MCP 응답 2MB 상한+소켓 abort `laf:mcp_response_too_large`, 타임아웃 `laf:mcp_timeout` — 50MB 실측 RSS +250MB → +14~30MB. F6 Drive를 `rest-support`로(302 거부, HTML 200은 `isError`). F7 지메일 동시성 6 + 호출당 30s 기한(50건 2,000ms → <1,500ms). §1 중복 개념 6개를 하나로(`plugin-one-shape.test.ts`가 재중복 방지). 게이트 server 1867 | 발견: `connections.tsx connectionFailureText`가 서버가 보내지 않는 코드로 키를 잡음(앱, 후속) |
| 2026-09-10 | 232a8eb | **감사 1파동 W1-g — 의존성·이미지**: (감사 A7) hono 권고문 넘겨 올림 + `bun audit` high 정리, Dependabot(npm·docker·actions·cargo, 주간·묶음) + 저장소 취약점 경고 on, 베이스 이미지 다섯 개 digest 고정, 서버 이미지 다단계 슬림(불필요 ~900MB 제거 — 실측 크기는 커밋 본문), 같은 커밋 두 번 빌드의 digest 차이와 이유 기록, `GET /api/version`(IMAGE_TAG+revision) + 앱 설정·도움말 푸터의 버전 표시 | 자세한 수치는 커밋 `fc422bd` 본문과 `~/laf/activity.md` |
| 2026-09-13 | 1aec32c | **감사 1파동 W1-a 후속 — 플러그인 경로도 같은 문**: `GET /api/plugins/for/:agentId`, `POST /api/plugins/for/:agentId/skills/:slug/view`, `POST`/`DELETE /api/plugins/grants`를 `requireBotAccess`로 — 동료는 404 `laf:bot_not_found`, 익명 401, 관리자는 허용 목록대로(`/call`은 이미 스토어에서 거부). 권한 행렬 `STILL_OPEN` 비움, 실측 열린 셀 0. 게이트 server 1900 | Docker Desktop이 관리자 권한 창에서 멈춰 게이트는 임시 Postgres 16(포트 55432)에서 돌림 — 병합 뒤 CI가 17로 재검증 |
| 2026-09-13 | c335cb7 | **감사 1파동 W1-f — 루프는 답에 대해 거짓말하지 않는다**: (감사 A2) `agent-bot/src/index.ts`를 루프·서버·변환기·로그 파일로 분리(기존 39 테스트 그대로 통과). 실제 agent-bot 프로세스·실제 SDK·가짜 제공자로 재측정한 여덟 행: 정상 → done; 429 → 0/5/10s 3회 뒤 `laf:model_rate_limited`(retry-after 70이면 1.5s); 500 → 1.8s에 3회 뒤 `laf:model_unavailable`; 빈 답 → 낮은 effort로 재시도(사용량 이벤트 2); 깨진 인자·없는 툴 → 같은 실행 안에서 모델에 사실로 돌려주고 3회 뒤 그 사실로 실패(고치면 정상 종료); 60s 침묵 → keepalive 뒤 done, 영원한 침묵 → 120.1s에 `laf:model_timed_out`; 스트림이 답 도중 끊기면 `laf:provider_stream_cut`, 절반 답 보존·원장 `error`. 같은 호출 반복은 4번째 요청에서 멈춤. 기한 뒤 결정된 클릭은 가짜 컴퓨터에 닿지 않음. stall 감시 기본 on. `eval:deferral` 23,438 → 13,291B(−43%). 게이트 server 1907·root 220 | `copilot.ts`는 변경 없음(AbortSignal은 `forwardedProps`로 못 건넘) |
| 2026-09-13 | ce01197 | **감사 1파동 W1-e — 서버가 하나의 사실을 말한다**: (감사 A1, A5 §2·§7) ① `app.onError` — 실제 서버에서 감사관의 없는 봇 루틴은 404 `laf:bot_not_found`(로그 없음), 강제 FK 실패는 500 `laf:internal` + 로그 한 줄 `database error (23503)`(SQL·값 없음), 미지 자격증명 폐기 404, 행렬 500 0개. ② 산문 오류 21/22 → 코드(남은 1개는 `app.ts` 관리자 자격증명 산문), 코드 42개 신설·한국어, 경로 파일을 훑는 테스트가 main의 산문 72개 → 0. ③ 루틴 실행 원자성 — 실제 프로세스를 배달 직후 SIGKILL: 전엔 끝난 답을 "중단"으로 표시, 이제 배달 전 죽으면 정직하게 중단, 커밋 뒤 죽으면 답·원장 finished·영수증이 남고 보고 없음. ④ 삭제의 함대 통지는 삭제 트랜잭션 안에서 아웃박스 행으로 — 함대 503·kill·재시작 뒤 한 번 배달. ⑤ 턴당 최악 스레드 읽기 5,001행 → 500(추가)·200(방 프롬프트·개인 이력), 보존은 `data-lifecycle.md` §4대로 나이 제한 없음(문서에 결정 기록). ⑥ 미리보기는 사람 단위, 마이그레이션 `0038_thread_preview_fleet_outbox`가 각자 스레드에서 채움. 게이트 3,003(server 1935·app 701·agent-computer 147·root 220) | 남은 산문 1개(`app.ts` 관리자 자격증명) — 2파동 |
| 2026-09-13 | a1348be | **감사 1파동 W1-b — 비밀은 모델·디스크·로그에 가지 않는다**: (감사 A3 S1, A8 S1·S2, A5 §6) ① `request_secret`로 친 값: 컴퓨터가 사람이 타이핑한 칸을 추적(페이지가 이름을 바꿔도)하고 서버가 같은 칸을 가림 — 감사관 페이지에서 `61a0fc8`은 `"패스워드","value":"PERSON-TYPED-SECRET-7788"` 누출, 이제 없음. ② 커넥터 OAuth 토큰 저장 시 암호화, 기존 행은 부팅 때 암호화, `LAF_TOKEN_ENCRYPTION_KEY` 없으면 서버가 안 뜸 — 가짜 제공자 로그인 뒤 `accounts` 덤프에 원문 토큰 없음. ③ 로그 스크러버가 감사관이 찾은 12종을 전부 자름(테스트 표). ④ CSP 위반 0(Chromium 19화면 + 실제 대화 턴, 셸 페이지 0) — 트레이드오프: Zod의 `eval` 탐지 끔, 샌드박스 컴포넌트가 페이지 정책을 물려받아 `script-src`에 인라인 허용; WebKit(맥 셸)은 미측정. HSTS·`frame-ancestors 'none'`·nosniff. ⑤ 20MB 트리거 본문 2ms에 413, 대화 턴은 스레드를 싣기 때문에 32MB. ⑥ `update_profile` 정규화. 마이그레이션 없음. 게이트 server 1973·app 701·agent-computer 157·root 257 | 로컬 `.env`에 키 추가. 고객 VM은 함대(c2483e0)가 개통·env push 때 심음 — 시험 VM은 다음 `laf env push` 필요 |
| 2026-09-13 | 400ce4f | **감사 1파동 W1-d — 브라우저는 root가 아니고, 리다이렉트를 끝까지 본다**: (감사 A5 §1 S1, A3 S2 ×3) ① Chromium을 `pwuser`(uid 1001) + Playwright seccomp 프로필 + `chromiumSandbox: true`로 — 없으면 Playwright가 스스로 `--no-sandbox`를 붙인다(WIP가 놓친 것); 렌더러가 자기 네임스페이스를 가짐; 메모리 naver 312→317MiB·뉴스 343→360. ② 리다이렉트는 CDP Fetch로 홉마다 판정(WIP의 `route()`는 리다이렉트를 못 봄) — Docker 네트워크에서 배포 내부로 가는 모든 홉 403, 내부 대상은 접속조차 안 됨; 감사관의 URL 열 개가 테스트 표. ③ 라벨이 바뀐 클릭은 409로 아무것도 안 누르고, 다시 스냅샷하면 새 라벨로 승인을 물음. ④ 봇 5개×내비게이션 20회 → 삭제: 568→173MiB, pid 290→9, 프로필 0; reset이 디렉터리를 되살리던 버그와 좀비 프로세스 누수도 고침; 유휴 종료는 문서대로 10분(실측 597~630s). `checks.yml`에 userns sysctl 단계 추가. 게이트 3,150(server 2004·app 701·agent-computer 188·root 257) | Docker Desktop이 끝내 안 떠 로컬 Ubuntu 24.04 arm64 VM(Docker 29.8)에서 측정; 4회 중 1회 페이지 타임아웃 8건; CI sysctl 단계는 첫 실행 확인 필요 |
| 2026-09-13 | 58918bd | **감사 1파동 W1-i — 나쁜 날의 앱, 렌더하는 테스트**: (감사 A4 S1·S2 ×6, A6 §3·§4) 다시 시도가 질문을 한 번만 저장(그리고 main에 있던 버그 — 실패한 질문이 새로고침에 사라지며 다시 시도 버튼도 사라짐 — 수정). 401은 돌아올 경로와 함께 로그인 문으로, `/api/me` 403은 그 사실로, API 부재는 "서버에 닿지 못했습니다". 폴링(Chromium, 유휴 대화) 보이는 창 53 → 5회/분, 숨은 창 20 → 0, API 240s 정지 동안 266 → 17요청(대기 30→60s, 포커스 5s 안에 재개). 로그인 뒤 홈 첫 화면 JS 46파일·919kB → 36파일·327kB — 렌더러(1,050kB)·CopilotKit(874kB)는 대화에서만 로드, 회귀하면 `vite build`가 실패. 라이브 화면 소켓 백오프 재연결·끊김 표시, 알림 제목의 을/를·이/가, `connectionFailureText`가 서버가 실제로 보내는 코드로, `scripts/test-ci.ts` 테스트 파일 매니페스트(파일이 사라지면 실패 — 스크래치 삭제로 증명), 소스 grep 테스트 10개를 렌더 테스트로. 새 렌더 테스트는 매니페스트를 빼고 전부 `61a0fc8`에서 실패함을 확인. 게이트 3,217(server 2004·app 768·agent-computer 188·root 257), app 플로어 673→744 | |
| 2026-09-14 | 79cc118 | **감사 2파동 W2-e — 루틴 서비스 분리**: (감사 A1 §5) `routines/service.ts` 1,195 → 277줄(같은 export·타입, 지금 실행·웹훅만 남음). 새 모듈: schedule 210 · ticker 178 · run 279 · settlement 187 · run-report 59 · receipts 69 · store 323 · ownership 47 · errors 36. 80줄 넘는 함수 6개(createRoutineService 730, executeNow 299, createRoutineRoutes 163, createRoutineSuggestionService 118, create 95, tickOnce 83) → 0(최장 70). 실제 서버+agent-bot으로 분리 전후 기록 동일. 게이트 server 2004 | routines 아래 `process.env` 읽기 없음 |
| 2026-09-14 | 33f5260 | **감사 2파동 W2-d — 채널 라우트 분리**: (감사 A1 §5) `channels/routes.ts` 1,380 → 129줄, 여섯 라우트 모듈을 붙이기만 함, 스토어 조립은 `store.ts`; `createChannelRoutes` 이름·시그니처 그대로. 80줄 넘는 함수 6개(514·450·198·122·120·87) → 2개(`createStallGuard` 198 — 공유 상태 위의 작은 함수 묶음, `classifyTurnFailure` 87 — 순서가 곧 규칙인 목록). 중복 모음: 사람의 1:1 대화 찾기(`soloConversationOf`), 오류 매퍼 하나, 방 턴 입력 타입은 방 서비스에서. 옛/새 라우터에 요청 59,280개를 흘려 동일(실제 방 서비스가 만들 수 없는 1케이스 제외), 실제 서버 34/34. 게이트 3,217 | 로스터 행 작성기 중복은 `routines/`·`rooms/`에 있어 범위 밖 |
| 2026-09-14 | 6f3c7b6 | **감사 2파동 W2-a — 컴퓨터 게이트웨이 분리**: (감사 A3 §7) `computer/gateway.ts` 1,991 → 173줄(분리선을 적은 머리말, 다른 디렉터리가 가져가던 이름은 전부 그대로 export). `gateway/` 모듈: govern 484 · trail 312 · acts 270 · secrets 230 · navigation 192 · intent 147 · handovers 145 · snapshots 125 · caller 110 · addresses 94. 최대 함수 `createComputerGateway` 1,136줄 → `govern` 410(일부러 한 덩어리). 분리 커밋은 테스트 무변경. 컴퓨터 라우트·클라이언트의 영어 오류 43곳 → 0, `error-codes.test.ts`가 `computer/`까지; 앱의 세 화면(가림 칸·경계·컴퓨터)이 코드 표로. 실제 agent-computer로 호출해 보다 발견·수정: 풀리지 않는 주소를 페이지 타임아웃으로 보고하던 것 → `laf:page_failed`. 채널·루틴 분리 위에 리베이스한 게이트 server 2029·app 772 | `auth/guards.ts`의 `requireAdmin`이 아직 영어(W2-f로) |
| 2026-09-14 | 438c79c | **감사 2파동 W2-c — 합성 루트와 설정 하나**: (감사 A1 부록·§6) `server/src/main.ts` 1,501 → 808줄 — 배선이 아닌 것은 기능 옆이나 `server/src/boot/`로(라이브 화면 프록시는 루트 `live-screen.ts`), `index.ts`(eventsource 먼저) 그대로. `config.ts` 밖 `process.env` 읽기(구문 트리로 셈) 21 → 2: `telemetry-off.ts`(벤더 라이브러리 스위치, 영구), `computer/assignment.ts`의 `BOT_SEATS_PER_ACCOUNT`(임시 허용 목록). 부팅 로그 비교: 새 DB·크래시 뒤·최소 설정 세 번씩 전후 — 이벤트·내용·HTTP/소켓 프로브 동일. 의도한 변화: 잘못된 `PORT`·웹훅 URL·`OPENAI_BASE_URL`·`AUDIT_RETENTION_DAYS`는 부팅을 멈춤, 모르는 `BOT_TIME_ZONE`은 여전히 폴백하되 경고. `.env.example`·deploying 표·compose가 `config.ts`와 어긋나면 실패하는 테스트. 게이트 server 2078·app 772 | |
| 2026-09-14 | 2a3bffa | **1파동 잔여 R1·R2·R4**: R1 `/sign`이 better-auth의 영어를 보여 주고 콜백으로 돌아온 거절은 아무것도 안 보이던 것 → 오류 코드만 읽어 한국어 10문장 + 일반 1문장, 콜백은 `/sign`으로; 로컬 프로덕션 스택 Chromium에서 4번째 로그인 시작 429 → 한국어 확인, 취소·이메일 없음·명단 밖도 표시; 렌더 테스트 72케이스(옛 코드에서 전부 실패). R2 업스트림 없을 때 Caddy의 빈 502 → 503 JSON(`/health`·`/api/health` `{status:"down",checks:{api:"unreachable"}}`, `/api/*` `laf:api_unreachable`), 앱 `queries.ts`가 코드를 읽어 장애 때 `/sign`으로 보내지 않음; 실제 이미지로 전후 curl은 커밋 본문. R4 Dependabot 실패는 보안 업데이트 GHSA-wrw7-89jp-8q8g — Tauri 2.11.5의 GTK3 체인이 glib 0.18에 고정 → ignore + `deploying.md` 기록(경고는 오너가 dismiss해야 초록). 게이트 server 2078·app 781·root 260 | 남은 것: 삭제된 기존 계정이 날 403 JSON을 받음(서버, W2-f), compose 주석의 "502" |
| 2026-09-14 | 798a7ab + laf-control 8283b7f | **R5 — 설치본과 업데이트가 GitHub 대신 정문에서**: 저장소 비공개 뒤 앱 업데이트 주소(`github.com/…/releases/latest/download/latest.json`)가 익명 404 — 설치된 앱이 영영 업데이트되지 않고 새 고객이 받을 곳도 없던 것. 이제 `https://agent.laf-co.com/desktop/`: 정문 VM `/srv/desktop/<version>/`(한 번 놓이면 불변) + `current` 심볼릭 링크 + 고정 이름 `LAF-Agent-mac.dmg`·`LAF-Agent-windows.exe`·`latest.json`, 최신 3개 보관, Caddy 읽기 전용(`laf entry`가 유지). CI 업로드는 전용 계정 `laf-desktop`(sudo·docker 없음)의 키 하나 — `restrict,command="/usr/local/lib/laf-desktop/receive"`(명령 무시·터미널·포워딩·sftp 거부); `laf desktop door`가 키를 만들어 공개키를 설치하고 비밀키·known_hosts를 GitHub 시크릿(`DESKTOP_DOOR_SSH_KEY`, `DESKTOP_DOOR_KNOWN_HOSTS`)에, 지문을 변수로 — 로컬 비밀키 삭제. `tauri.conf.json` 업데이트 주소 교체, 초안 GitHub 릴리스는 폐기(서명 키를 가진 작업에 쓰기 권한 불필요). 실측: `curl -sI …/LAF-Agent-mac.dmg` 200·5,596,715B(아티팩트와 같은 바이트), windows 200 | `latest.json`은 `v*` 태그 빌드가 서명해야 생김 — 그 전엔 0.2.0 설치본만(업데이트 없음); 오너의 0.4.x 앱은 태그 뒤 한 번 재설치 |
| 2026-09-14 | 153e5c4 | **감사 2파동 W2-b — 브라우저 컨테이너 분리, 그리고 1/4의 타임아웃**: (감사 A3 §7) `agent-computer/src/index.ts` 2,375 → 124줄(배선) + 모듈 21개(navigation 408 · actions 258 · secret-fields 220 · sessions 188 · routes 161 · 나머지 33~144). 최장 함수 `fetch` 888줄 → `navigate` 148. 기존 188 테스트 무변경 통과. 컨테이너의 영어·Playwright 오류 본문 ~30곳 → `laf:` 코드(신규 13, 한국어) — **전엔 실패한 `/type`·`/human/secret`이 입력한 글자를 오류 본문에 돌려보냈다**, 이제 입력값은 돌아오지 않음(예외: `page_timeout`의 Playwright 첫 줄 — 서버 `client.ts`가 504 판정에 씀). **W1-d의 "4번 중 1번 타임아웃 8건"은 우리 버그**: 끝난 브라우저의 파이프를 Bun이 메모리 정리 때 한 번 더 닫는데 그땐 그 파이프 번호가 다음 브라우저 것 — 강제 정리하면 새 브라우저 9/9 사망; 수정 + 수정 없이 3/3 죽는 테스트, 수정 뒤 12회·1,200/1,200 내비게이션·타임아웃 0. 게이트 agent-computer 212·root 271 | `tool-results.ko.ts` 충돌은 서버(W2-a)·컨테이너(W2-b) 코드 표 둘 다 보존 — 두 어휘가 겹침(`page_failed`/`navigation_failed`, `workspace_*`/`file_*`) → 통일 후속 |
| 2026-09-14 | 8272cea | **감사 2파동 W2-f — 서버의 오류 언어 하나**: `server/src`(설정·합성 루트 제외) 산문 오류 64 → 0, `error`에 코드는 있는데 `code` 필드가 없던 본문 7 → 0, 권한 행렬에 "코드 없는 거절 셀" 검사 추가 224 → 0. 실제 서버 전후: 익명 `GET /api/me` 401 `"Authentication required."` → `laf:unauthenticated`; 비관리자 403 → `laf:admin_required`; 없는 id 404 `"No such tool"` → `laf:tool_unknown`, 서버 → `laf:server_unknown`, 컴포넌트 → `laf:component_unknown`. 코드 42개 신설(한국어), better-auth 자체 429도 `laf:rate_limited`, R1의 로그인 표는 `laf:sign_in_not_admitted`로. 54파일. 상태 변화 하나: `POST /api/plugins/servers`의 쓸 수 없는 토큰 500 → 400. 게이트 3,356 | |
| 2026-09-14 | bde43b4 | **감사 2파동 W2-g — 브라우저와 서버의 어휘 하나**: 컨테이너가 브라우저 안에서 일어난 일의 출처 — `agent-computer/src/codes.ts` 32개(실패 답 24·노트 6·소켓 2), 서버 클라이언트는 자기 것 4개만 더함(`computer_unreachable`·`computer_timed_out`·`computer_failed`·`url_invalid`). 겹치던 이름 6개 제거(`page_failed`·`workspace_path_refused`·`workspace_file_unusable`·`snapshot_stale`·`computer_unavailable`·`secret_field_gone`), 클라이언트는 `code`를 읽음(Playwright 첫 줄 매칭 제거). 계약 테스트: 컨테이너가 보낼 수 있는 코드를 서버·툴 결과 표·앱이 모르면 실패. 실측(워크트리 서버 → 워크트리로 빌드한 이미지): 없는 호스트 502 `navigation_failed`, 응답 없는 픽스처 30.4s 뒤 504 `page_timeout`, 숨은 버튼 409 `element_not_actionable`, 없는 파일 400 `file_not_found`, `../outside.txt` 403 `file_path_refused`, 틀린 토큰 503 `computer_token_refused` — 여섯 모두 모델이 받는 한국어 문장 확인. 실측에서 발견·수정: `display:none` 버튼이 `label_changed`로 답하던 것. 게이트 server 2115·app 803·agent-computer 217 | 남은 것: 감사 페이지가 컴퓨터 코드를 날것으로 보임; 실행 중인 서버 아래서 컨테이너를 갈아 끼우자 다음 호출 4개가 45s 기한까지 멈춤(원인 미조사) |
| 2026-09-14 | de340b9 | **3파동 W3-f — 업그레이드를 끝까지 시험**: `scripts/upgrade-e2e.ts` + `.github/workflows/upgrade-e2e.yml`(주간 + dispatch). `:stable`(v0.4.5)을 VM처럼 세우고 공개 API로 데이터를 심은 뒤 `scripts/upgrade.sh`로 이 커밋의 이미지로 올리고 13가지 확인: 36표 92행 불변, 마이그레이션 34→39 한 번, 옛 쿠키로 로그인 유지, 봇 브라우저가 옛 프로필을 엶, 가짜 모델로 한 턴 답, 업그레이드 전 덤프가 같은 수로 복원. 로컬 실측 두 번 모두 통과 — `upgrade.sh` 24.1/23.6s, 바깥 단절 `/` 3.0s·`/health` 14.3/14.8s; 발행된 `:edge`로는 44.7s·`/health` 16.3s. CI 예상 약 8분/회(월 35분, 러너에서 미측정). zizmor 발견 없음 | **발견**: `openbot-deploy:stable`이 없어 stable VM의 문서대로 묶음 추출이 실패(드라이버가 git으로 재구성하며 매번 보고); stable의 compose가 새 서버가 요구하는 `LAF_TOKEN_ENCRYPTION_KEY`를 넘기지 않음 — 둘 다 v0.5.0 태그로 `:stable`을 다시 찍으면 해소. 공유 4GB Docker VM에서 web 이미지 빌드 OOM 5번 → `--no-build` |
| 2026-09-14 | 9cfbede | **3파동 W3-a — 루틴 메모장("지난번 어디까지")**: (Hermes 대조 §4 1번) 루틴 자신의 실행에만 제공되는 `routine_note` 툴(`watermark`/`set`/`delete`, `skill_view`와 같은 규칙) — 최종 답 필드가 아닌 이유: 거절된 쓰기는 쓴 실행이 즉시 알아야 하는데 최종 답은 너무 늦다. 한도: 20항목, 노트 ≤500자, 키 ≤40자, 워터마크 = id(≤200자)·시간대 있는 시각, 다음 실행이 읽는 합계 4,096B; 값은 기억 저장소의 지시·비밀 검사(16자리 주문번호가 들어가게 id는 비밀 검사 제외), 거절은 `laf:notepad_*`로 실행에 즉시. 실제 agent-bot으로 두 실행: 둘째 프롬프트에 R-1002; 커밋 전 SIGKILL이면 커서는 R-1000; 실패한 실행은 아무것도 안 옮김; 실행 중 지우기가 그 실행의 쓰기를 이김; 채팅 턴은 `laf:tool_unknown`. 변이 둘(버전 검사 제거·트랜잭션 밖 쓰기)에서 테스트 실패 확인. 프롬프트: 한도까지 찬 메모장 +4,375B(시스템 메시지), 툴 정의 +996B(루틴 실행에만). 마이그레이션 `0039_routine_notepad`. 게이트 server 2161·app 809 | |
| 2026-09-14 | 0135bce + bf2371d | **3파동 W3-i — `laf insights`가 읽는 기록**: `POST /api/me/first-task` → 감사 `onboarding.first_task_pressed`(카탈로그 키만, 문장 없음); `agent_profiles.preset_id`(마이그레이션 `0040_insight_rows` — 사람의 생성·PATCH만 설정, 봇의 `update_profile`은 못 바꿈, 복제는 안 옮김); `support.help_opened {section}`(방문당 한 번); `GET /api/admin/metrics/insights?days=N` — 함대 토큰 `LAF_FLEET_METRICS_TOKEN`(상수 시간 비교, 없으면 404로 존재를 숨김), laf-control의 아홉 절과 같은 필드 이름 + `fromPreset`·`firstTasks`·도움말 수, 창은 `from ≤ t < to`. 실측(워크트리 서버·Vite): 리뷰·평판 프리셋 → `preset_id=social`, 칩 한 번, `/help`·`/help#routines` → 토큰 없이 401 `laf:fleet_token_refused`, 토큰으로 JSON(봇 1·칩 1·도움말 2), 변수 미설정 404 | 병합 때 마이그레이션 번호를 노트패드(0039) 뒤로 drizzle-kit으로 다시 사슬; 그 메타 JSON이 포맷 검사를 못 넘은 채 병합돼 bf2371d로 즉시 수정(병합 스크립트를 네 검사 모두 확인하도록 고침). 남은 것: 컴퓨터 쪽 `site.*` 행·`opaqueFrames`(W3-c 진행 중), 코드 없이 저장된 채팅 실패 1건 |
| 2026-09-14 | f5ade7a | **3파동 W3-b — 같은 실패는 한 번만 말한다**: (Hermes 대조 §4 2번) 실패 서명 = (루틴, 실패 코드, 코드가 가리키는 툴 — `laf:turn_tool_failed`만 툴을 가짐). 마이그레이션 없음: 첫 실패의 `run.failed` 아웃박스 행이 곧 그룹(`subject.group`에 횟수·마지막 시각·확인·닫힘), 실행 자신의 정산에서 기록; 반복은 새 줄·알림 없음; 성공이 그룹을 닫고 다음 실패는 한 번 알림; `POST /api/me/notifications/:id/acknowledge`로 "확인". 실측(실제 서버·agent-bot·항상 429인 제공자, 티커로, 5회째에 재시작): 전 10행·웹훅 10회·빨간 줄 10 → 후 1행·1회·"10번" 세는 줄 1(재시작을 건너서); 확인 뒤 조용, 성공 뒤 두 번째 알림; 한국어 Chromium에서 확인한 줄은 회색, 새로고침 뒤에도 회색. 루틴 메모장과 같은 정산을 건드려 충돌 — 한 트랜잭션에서 메모장 → 실패 그룹(성공이면 닫고 실패면 세거나 엶) → 대화·원장·영수증 순서로 합침, 둘을 함께 시험하는 테스트 추가(한쪽을 일부러 망가뜨리면 실패). 게이트 3,604 | 채팅 실패와 부팅 중단은 묶지 않음(사람의 질문마다 자기 줄, 재시작은 각자 사건) |
| 2026-09-14 | e7fb80e | **3파동 W3-c — 자동 심사 방어·사이트 기록·감사 코드·컨테이너 교체, 그리고 게이트의 JIT 버그**: ① 자동 심사가 페이지 데이터를 구분자 사이에 두고 괄호·닮은꼴을 이스케이프, 판정은 allow/ask만(그 밖은 ask) — 주입 라벨 6종 한 번도 통과 안 함, 실제 모델로 공격 18건 중 허용 0(전후), 무해 3/3. ② `site.signed_in`·`site.login_lapsed`는 바뀔 때만(사이트 id, URL 없음), 스냅샷의 `opaqueFrames` 수(0 초과면 행) — 실행 스택에서 확인. ③ 감사 화면이 `laf:navigation_failed` 대신 "페이지를 열지 못함", `codes.ts`의 모든 코드를 훑는 테스트. ④ 실행 중 서버 아래서 컨테이너를 갈아 끼우면 4호출이 45s씩 멈추던 원인은 재사용된 keep-alive 소켓 → `keepalive: false`, 대리 포워더로 측정 45.0s×4 → 첫 호출 0.39s 성공. **병합 중 발견**: 게이트의 "파일마다 테스트가 돌았는가" 검사가 뒤쪽 두 그룹의 모든 파일을 0으로 읽음 — 원인은 Bun 1.3.11 JIT 버그(그룹 실행이 ~30s 넘게 루프를 기다리게 하면 필터 클로저가 첫 그룹의 결과를 읽음; 30s 이상 대기 23회 중 21회 오판, 20s 이하 10회·JIT 끔·baseline JIT에선 0). 루프에서 클로저를 없애고 판정은 `scripts/test-ci-report.ts`의 평범한 함수로, 보고서가 bun이 센 수와 정확히 맞아야 함; 새 테스트가 까다로운·빈 파일로 bun을 실제로 돌림; 재현 12/12 정답. 증명: 게이트 초록 두 번, 테스트 파일 하나를 비우면 "root: 1 test file(s) ran no test at all". 게이트 server 2306·root 308 | 알려진 흔들림: 부하에서 `site-connections.test.ts:367`(31ms 차이 타임스탬프) 한 번; 스냅샷이 끝내 안 뜨는 iframe에서 영원히 멈춤(미수정) |
| 2026-09-14 | dab7754 | **문의 상자의 진단 정보(선택)**: (Hermes 대조 §4 8번) "진단 정보 같이 보내기"를 체크해야만 서버가 묶음을 만듦 — 앱 버전, 그 사람 자신의 봇·턴에 속한 최근 JSON 로그 이벤트 50개(이벤트 이름·코드·시간만, 스크러버를 한 번 더, 입력·메시지·쿼리 있는 URL·이메일·토큰이 들 수 있는 필드는 버림), 최근 실패 코드와 횟수, 건강 스냅숏; 보내기 전에 한국어로 무엇이 가는지 펼쳐 보여 줌; 전체 묶음은 피드백 행에(VM에 남음), 함대 웹훅에는 수만. 실측(Chromium, 두 사람): 체크 전엔 아무것도 안 가져옴, 미리보기에 오너 이벤트 4개, 직원의 id 없음, 오너가 친 비밀번호·이메일·토큰 URL 없음. 삭제된 계정이 날것 JSON을 보던 문제는 이미 W2-f로 해소 확인(`/sign?error=laf:sign_in_not_admitted` → 한국어) | **발견**: 제거된 사람의 기존 세션이 계속 동작(`/api/me` 200, 쓰면 갱신) — 수정 착수 |
| 2026-09-14 | dbc1c67 | **정리 — 죽은 iframe·흔들린 테스트·502 주석**: ① 로드되지 않는 iframe이 있는 페이지에서 `/snapshot`이 120s 넘게 무응답(두 번) — 기한이 아예 없었음 → 15s 기한(`browser-limits.md`), 실측 200·9.17/9.23s·`opaqueFrames: 1`·나머지 페이지 목록; **기한을 넣자 늦게 도착한 프레임이 비밀번호 값을 흘릴 수 있어** 늦은 프레임을 다시 검사, 그 검사가 없으면 실패하는 테스트. ② `site-connections.test.ts:367`은 두 시계(Bun·Postgres)의 타임스탬프 비교 — 코드가 시계를 주입받지 않아 허용 오차로, 시계를 31ms 앞으로 두면 옛 단언은 매번 실패; app·agent-computer를 옆에서 돌리며 20/20. ③ `/health`가 API 부재 시 502라던 주석·문서(compose·deploying·upgrade.sh와 그 테스트·주석 셋)를 503 JSON으로 | **발견**: 끝내 응답 없는 사이트로 이동 중인 탭에서 `/read`가 멈춤 → 착수 |
| 2026-09-14 | 73ae710 | **제거된 사람의 세션은 즉시 끝난다**: 원천 = 서버가 부팅한 `SIGN_IN_ALLOWED_EMAILS`(laf-control이 명단을 거기 쓰고 컨테이너를 재시작) + 세션 조회가 이미 읽는 사용자 행. 명단에서 빠진 세션은 삭제되고 401 `laf:session_revoked`, 제거 시 라이브 화면·활동 소켓도 닫음. DB 조회 추가 없음 — 검사 0.03µs, `/api/me` p50 2.45 → 2.53ms(잡음 범위). Chromium 한국어로 세 경우 모두 로그인 문 + 한국어 문장: 오너가 삭제 후 새로고침, 명단 축소+재시작 후 새로고침, 탭을 연 채 삭제(새로고침 없이). 게이트 server 2337 | 정정: `dab7754`에서도 관리자 삭제는 이미 401이었다 — 뚫린 것은 명단 경로뿐 |
| 2026-09-14 | a1fada9 | **이동 중인 탭은 읽기를 막지 않는다**: 원인 — 탭의 내비게이션이 끝나지 않으면 Chromium이 페이지에 대한 모든 질문을 착지까지 붙잡고, 끝내 응답 없는 사이트는 착지하지 않음. `/navigate`→`/hang` 1s 뒤 전후(재빌드 컨테이너): `/read` 29.1s에 502 → 1.03s에 200 + 새 노트 `laf:page_loading`; `/snapshot` 12.0s 502 → 1.01s 200; `/screenshot` 29.1s 502 → 52ms 200; `/describe-point` 29.1s 502 → 1.01s 200; `/click`·`/type` 29.1s 502 → 1.01s 409 `laf:stale_refs`; `/tabs/switch` 제목이 `Loading …/hang`이던 것 → 빈 제목. **비밀값이 탭 제목으로 새던 경로 발견·수정**: 비밀을 친 뒤 GET 폼이 `/hang`으로 가면 `/tabs/switch`가 제목에 그 값을 담음 — 테스트 추가. 게이트 agent-computer 228 | 에이전트가 초기에 Playwright 브라우저에 넓은 `pkill`을 써서 다른 워크트리 테스트 브라우저를 죽였을 수 있음(그 뒤 병합 게이트는 모두 초록) |
| 2026-09-15 | a48d131 | **셀프서브 W5 — 무료 체험이 제품 안에**(계약 `~/laf/docs/self-serve-2026-09-15.md` §14 W5): VM `.env` 넷 `LAF_PLAN`·`LAF_TRIAL_ENDS_AT`·`LAF_TRIAL_HOLD_DAYS`·`LAF_DAILY_TOKEN_BUDGET`(넷 다 아니면 이름을 대고 부팅 거절, `config.ts`·compose·`.env.example` 같은 이름), 체험 남은 날 배너, `/sign?via=` 자동 시작, 하루 토큰 예산(한국 시간 자정 기준, 채팅·방·루틴·봇 간 부탁·서버 모델 호출 모두, 거절 `laf:daily_budget_reached` + 한국어 문장), 약관 HTML은 CI(`images.yml`)가 `render-legal.ts`로 굽고 `deploy/Dockerfile`은 COPY만, 약관·처리방침에 체험 조항 **초안**(`LEGAL_VERSION`은 오너 법률 확인 뒤), 데스크톱 셸 오프라인 화면이 정문 `dest-status`로 보관 중 문장. 에이전트 실측: 가짜 모델·예산 1 → `/api/me` 네 값, 첫 턴 `model.usage` 1행(1,230 토큰), 둘째 턴은 agent-bot에 닿지 않고 한국어 거절(새로고침 뒤도). 게이트 server 2402·app 886·agent-computer 228·root 330 | **발견·수정**: 방·루틴·봇 간 부탁의 모델 사용량이 한 번도 기록되지 않았다 — `model.usage`를 쓰던 러너는 채팅 엔드포인트만 지남. 기록을 모든 실행이 지나는 `copilot.ts`로 옮김. W4에 넘길 것: 셸의 `dest-status` 호출은 교차 출처(CORS 필요), 계약 예시 슬러그 `k3m9x2qa7b`는 base32가 아님 |
| 2026-09-15 | 0ad1f1e | **이미지 푸시 잡에 캐시를 되살리지 않는다**: W5가 넣은 `setup-bun`이 기본값으로 캐시를 켜 zizmor가 병합 푸시를 실패시킴(cache-poisoning, high). `no-cache: true`(release.yml과 같은 이유). 번들 테스트는 Bun 판이 검사와 같은지와 캐시가 꺼졌는지를 따로 단언 — 줄을 빼면 실패. 같은 zizmor 이미지 다이제스트로 로컬 0 findings(18 suppressed) | 첫 게이트에서 W5 테스트의 "`with` 블록 전체가 검사와 같아야"가 수정을 막음 → 의도(같은 Bun)와 예외(캐시 없음)로 나눔 |
| 2026-09-15 | 72bdcd5 | **루틴 실행 기록의 시간이 "21s"로**: `runShape`가 턴·도구는 `t()`로, 초는 `${seconds}s`로 붙였다. 정부지원사업 첨부용 스크린샷을 찍다가 발견 — `t`를 인자로 받는 함수라 i18n 커버리지 테스트가 못 봄. 한국어 `t`로 `2턴 · 도구 1개 · 21초` 단언(수정 전 `21s`로 실패 확인), 기존 키 `{seconds}s`→`{seconds}초` 재사용. 게이트 server 2402·app 887·agent-computer 228·root 330 | — |
| 2026-09-16 | bc5bf3e | **봇의 브라우저 프로파일은 배포의 것**(오너 결정, `profiles.ts`의 "One profile per Bot"을 뒤집음 — "한 봇이 로그인한 사이트는 다른 봇도 로그인해야 해. 같은 걸 여러 번 허용하게 하지 마"): 크로미움 사용자 디렉터리 하나, 쿠키 항아리 하나, **탭은 봇마다 그대로**(bot-lane이 봇별 큐). 실측(재빌드 이미지·전용 컨테이너 4199): shop-bot 로그인 → office-bot이 같은 세션, reset은 `scope: deployment`. 업그레이드는 가장 최근 프로파일을 그 자리에서 이어받고 나머지는 둔다(`laf:profile_adopted`). 게이트 server 2402·app 887·agent-computer 242·root 330 | **내준 것**: 봇별 사이트 허용의 과속방지턱, `EGRESS_PROXY_<BOT>`(부팅 경고), reset은 전원 로그아웃, 봇 삭제가 로그인을 안 가져감(VM 파기가 가져간다) |
| 2026-09-16 | 32a87c1 | **봇은 만든 계정의 것 — 공개/비공개 구분 삭제**(오너: "남이 만든 봇을 다른 계정이 볼 수 있는 구조 자체가 잘못"): 마이그레이션 0042가 `agent_profiles.visibility`와 `agent_visibility` 타입을 지우고(40개 DB에 public 0행 확인) `(owner_user_id, deleted_at)` 인덱스를 둔다. 규칙은 `profile-policy.ts` 한 곳 — 주인이거나 주인 없는 패키지 봇(오늘 0개). id로 찌르는 경로 전부 404: 봇 API·기억·복제·숨김·방 만들기/초대, 모든 실행이 지나는 AG-UI 명단, 루틴 범위. 게이트 server 2411 | 화면에서 공개 선택지·문구·`laf:agent_visibility_invalid` 삭제 |
| 2026-09-16 | 397213f | **봇을 조종하는 것도 주인만** — `actorMayDriveBot`의 관리자 예외 삭제. 권한 행렬 23칸이 관리자에게 닫힘(화면·조종·파일·스냅샷·시연·열린 승인·권한·컴포넌트), `plugins/skills-and-grants.ts`의 사본도. 게이트 server 2413 | 대가: 남의 봇이 승인 질문에 멈추면 운영자는 못 푼다 — 기록·정책·상시 허용 회수까지만, 그다음은 VM 재시작이나 그 사람 삭제(`docs/laf/coworkers.md`) |
