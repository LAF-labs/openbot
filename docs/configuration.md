...
| `INITIAL_ADMIN_EMAILS`                                  | The address the deployment belongs to. It becomes the administrator of the 관리 menu the first time it signs in, and it is admitted even when `SIGN_IN_ALLOWED_EMAILS` leaves it out. |
| `SIGN_IN_ALLOWED_EMAILS`                                | The door. A deployment belongs to exactly one account: with `NODE_ENV=production` the server refuses to start unless this and `INITIAL_ADMIN_EMAILS` name one address between them, and that address closes the door even when only the admin line names it. Unset outside production is an open door, said at boot. In every environment, sign-in refuses a second person once the deployment has an account it still admits (`laf:deployment_has_account`). |
