...
import { MAX_SIGNALS, normalizeSignals } from "../src/watch/differ";
...
if (!url) {
  console.error(
    "사용법: bun server/scripts/laf-mcp-check.ts <url> [--token <bearer>] [--json]",
  );
  process.exit(64);
}
...
try {
  await Promise.race([
    ...
    new Promise((_resolve, reject) =>
      setTimeout(
        () =>
          reject(new Error(`${CONNECT_TIMEOUT_MS / 1000}초 안에 답하지 않음`)),
        CONNECT_TIMEOUT_MS,
      ),
    ),
  ]);
  ...
  try {
    ...
    if (result.isError) {
      ...
      if (parsed !== undefined) {
        ...
        if (rawCount === 0) {
          record("warn", "signals", "빈 배열 — 감시할 것이 없다는 뜻인지 확인");
  ...
  if (json) {
    ...
    console.log(
      failures.length === 0
        ...
        : `\n등록 불가 — FAIL ${failures.length}건을 고칠 것. (계약: docs/laf/mcp-contract.md)`,
    );
  }
