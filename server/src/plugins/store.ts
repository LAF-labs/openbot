 ...
 * {@link createPluginStore}'s `refuseAndReplaceEvictedClient` hung on a substring of prose written
 * for a person to read: rewording the sentence — translating it, dropping the parenthesis — would
 * have turned self-registration off with every test still green. A field cannot be reworded by
 * accident.
 */
    ...
    async disconnectAccount(input: {
      ...
      if (entry?.auth.kind === "user-oauth") {
        ...
        try {
          ...
          const client = await storedOAuthClient(input.serverId);
          const params = new URLSearchParams({
            token: refreshToken,
